...
package zipcar_marketingdw.process_rfm_member_data_0_1;
...
/**
 * Job: Process_RFM_Member_Data Purpose: Computes RFM member values<br>
 * Description: Computes RFM member values from reservation data <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Process_RFM_Member_Data implements TalendJob {
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Process_RFM_Member_Data";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_dcPLYNzGEeadL7U1585bjw", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Process_RFM_Member_Data.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Process_RFM_Member_Data.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
Logging.componentName(jobName, "tWarn_1", "RFM Query");
Logging.componentName(jobName, "tWarn_2", "Log_RFM_Rows");
Logging.componentName(jobName, "tWarn_3", "Log_Reset_RFM_Rows");
Logging.componentName(jobName, "tWarn_4", "Log_RFM_Weight");
Logging.componentName(jobName, "tWarn_5", "Log_RFM_Query_Failure");
...
globalMap.put("START_DATE", DateUtils.startDate(context.RunDate, context.RfmWeeksOffset));
globalMap.put("END_DATE", DateUtils.endDate(context.RunDate));
globalMap.put("Rfm_New_Processed", new java.util.concurrent.atomic.AtomicInteger(0) );
...
public static class deleted_rowsStruct implements routines.system.IPersistableRow<deleted_rowsStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_RFM_Member_Data = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Process_RFM_Member_Data = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Process_RFM_Member_Data.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Process_RFM_Member_Data.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_RFM_Member_Data = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_RFM_Member_Data = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Process_RFM_Member_Data, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Process_RFM_Member_Data, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_RFM_Member_Data) {
...
"\nDELETE FROM {{marketing.schema}}.s_rfm  \nWHERE end_date = {{endDate}}\n",
...
if(deleted_rows != null) { 
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_3", "", Thread.currentThread().getId() + "", "INFO","","Deleted Current Day RFM Records for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_3", 3, "Deleted Current Day RFM Records for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_3_WARN_MESSAGES", "Deleted Current Day RFM Records for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE"))); 
...
"\nINSERT INTO {{marketing.schema}}.s_rfm (person_id, member_id, recency, frequency, monetary, weighted_rfm, profile, start_date, end_date)\nWITH  account_member AS (\n	SELECT DISTINCT dm.member_id, dm.person_id\n	FROM {{marketing.schema}}.d_member AS dm\n	WHERE dm.start_date <= {{endDate}}\n	AND dm.end_date >= {{endDate}}\n	AND dm.is_active = TRUE\n	ORDER by person_id\n),\n recency_data AS (\n	SELECT member_id, {{marketing.schema}}.f_rfm_recency(nvl(days, 183)) as recency\n	FROM (\n		SELECT member_id, datediff(days, recency_date, {{runDate}}) AS days\n		FROM (\n			SELECT member_id, max(start_date) AS recency_date\n			FROM {{marketing.schema}}.s_reservation  fr\n			WHERE member_id in (select member_id from account_member)\n			AND fr.start_date >= {{startDate}}\n			AND fr.start_date < {{runDate}}\n			AND (fr.cancel_date is null \n				OR fr.cancel_date >= {{runDate}}) \n			GROUP BY member_id\n		) \n	)\n),\nfrequency_data AS (\n	SELECT member_id, {{marketing.schema}}.f_rfm_frequency(nvl(median, 56)) as frequency\n	FROM (\n		SELECT member_id, MIN(frequency_median) AS median\n		FROM (\n   			SELECT member_id, median(frequency_days) OVER (PARTITION BY member_id) AS frequency_median\n			FROM (\n				SELECT member_id, datediff(days, fr.start_date, lead(fr.start_date, 1) OVER (PARTITION BY member_id ORDER BY fr.start_date)) AS frequency_days\n				FROM {{marketing.schema}}.s_reservation fr\n				WHERE member_id in (select member_id from account_member)\n				AND fr.start_date >= {{startDate}}\n				AND fr.start_date < {{runDate}}\n				AND (fr.cancel_date IS NULL \n					OR fr.cancel_date >= {{runDate}}) \n			)\n		)\n		GROUP BY member_id\n	)\n),\nmonetary_data AS (\n	SELECT member_id, {{marketing.schema}}.f_rfm_monetary(nvl(hours, 0)) as monetary\n	FROM (\n		SELECT member_id, SUM(fr.total_hours) AS hours\n		FROM {{marketing.schema}}.s_reservation  fr\n		WHERE member_id in (select member_id from account_member)\n		AND fr.start_date >= {{startDate}}\n		AND fr.start_date < {{runDate}}\n		AND (fr.cancel_date is null\n			OR fr.cancel_date >= {{runDate}}) \n		GROUP BY member_id\n	)\n)\nSELECT am.person_id, am.member_id, \n	nvl(rd.recency, 1) AS recency,\n	nvl(fd.frequency, 1) AS frequency,\n	nvl(md.monetary, 1) AS monetary,\n	{{marketing.schema}}.f_round_he(wt.recency * nvl(rd.recency, 1) + wt.frequency * nvl(fd.frequency, 1) + wt.monetary * nvl(md.monetary, 1), 0) AS weighted_rfm,\n	{{marketing.schema}}.f_rfm_profile(cast({{marketing.schema}}.f_round_he(wt.recency * nvl(rd.recency, 1) + wt.frequency * nvl(fd.frequency, 1) + wt.monetary * nvl(md.monetary, 1), 0) as integer)) as profile,\n	{{startDate}} AS start_date,\n	{{endDate}} AS end_date\nFROM account_member AS am\nLEFT JOIN recency_data AS rd on rd.member_id = am.member_id\nLEFT JOIN frequency_data AS fd ON fd.member_id = am.member_id\nLEFT JOIN monetary_data AS md ON md.member_id = am.member_id\nJOIN {{marketing.schema}}.l_rfm_weight wt ON wt.start_date <= {{runDate}} AND wt.end_date >= {{runDate}}\n",
"marketing.schema", StringHandling.DQUOTE(context.Redshift_Marketing_Schema),
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_1", "", Thread.currentThread().getId() + "", "INFO","","Member RFM data inserted into (s_rfm) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_1", 3, "Member RFM data inserted into (s_rfm) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_1_WARN_MESSAGES", "Member RFM data inserted into (s_rfm) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE"))); 
 ...
 */
    ...
    public static void main(String[] args){
        final Process_RFM_Member_Data Process_RFM_Member_DataClass = new Process_RFM_Member_Data();

        int exitCode = Process_RFM_Member_DataClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Process_RFM_Member_Data.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/process_rfm_member_data_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Process_RFM_Member_Data");
        }
...
/************************************************************************************************
 *     129350 characters generated by Talend Open Studio for Data Integration 
