...
package zipcar_marketingdw.process_monthly_member_details_0_1;
...
/**
 * Job: Process_Monthly_Member_Details Purpose: Computes monthly member details<br>
 * Description: Computes monthly member details <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Process_Monthly_Member_Details implements TalendJob {
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Process_Monthly_Member_Details";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_MEj7cGW3Eei4tNjVPIAMGw", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Process_Monthly_Member_Details.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Process_Monthly_Member_Details.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
Logging.componentName(jobName, "tWarn_3", "Log_Reset_Records");
...
globalMap.put("START_DATE", DateUtils.getFirstDayOfMonth(DateUtils.getMonthlyEndDate(context.RunDate)));
...
public static class deleted_rowsStruct implements routines.system.IPersistableRow<deleted_rowsStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_Monthly_Member_Details = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Member_Details = new byte[0];
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_Monthly_Member_Details) {
...
"\nDELETE FROM {{marketing.schema}}.s_member_detail_monthly\nWHERE end_date = {{endDate}}\n",
...
if(deleted_rows != null) { 
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_3", "", Thread.currentThread().getId() + "", "INFO","","Deleted Monthly Member Detail Records for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_3", 3, "Deleted Monthly Member Detail Records for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_3_WARN_MESSAGES", "Deleted Monthly Member Detail Records for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE"))); 
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Member_Details.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Member_Details.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Member_Details = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Member_Details = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Member_Details, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Member_Details, 0, length, utf8Charset);
		}
		return strReturn;
	}
...
"\n-- get month details for last month\ncreate temp table member_detail_last_month as (\n	select distinct\n		member_id,\n		nvl(is_inactive, true) as is_inactive,\n		nvl(months_inactive, 0) as months_inactive\n	from {{marketing.schema}}.s_member_detail_monthly\n	where end_date = {{startDate}} - 1\n);\n\n--drop table if EXISTS month_reservations;\ncreate temp table month_reservations as (\n	select member_id, reservation_id, start_date, end_date, total_hours, total_distance, up_revenue, late_returns_p, currency_iso\n	from {{marketing.schema}}.s_reservation\n	where (end_date >= {{startDate}} and start_date < {{runDate}}) -- includes all reservations that start or end in the month\n	and cancel_date is null\n);\n\n--drop table if EXISTS month_reservation_totals;\ncreate temp table month_reservation_totals as (\n	select member_id, currency_iso,\n		count(distinct reservation_id) as total_reservations,\n		sum(total_hours) as total_hours,\n		sum(total_distance) as total_distance,\n		sum(up_revenue) as total_up_revenue,\n		sum(decode(late_returns_p, 't', 1, 0)) as total_late_returns,\n		sum(decode(end_date < {{runDate}}, true, 1, 0)) as trips_completed\n	from month_reservations\n	where (start_date >= {{startDate}} and start_date < {{runDate}}) -- only include reservations start in the month for totals\n	group by member_id, currency_iso\n);\n\n--drop table if EXISTS reservation_fractions;\ncreate temp table reservation_fractions\ndistkey (member_id)\nsortkey (reservation_id, day) as (\n	with digit as (\n		select 0 as d union all \n		select 1 union all select 2 union all select 3 union all\n		select 4 union all select 5 union all select 6 union all\n		select 7 union all select 8 union all select 9        \n	),\n	-- max 999, add more join tables to increase the max\n	seq as (\n		select a.d + (10 * b.d) + (100 * c.d) as num \n		from digit a\n		cross join\n		digit b\n		cross join\n		digit c\n		order by 1        \n	)\n	select\n		reservation_id,\n		member_id,\n		day,\n		hours,\n		round(total_distance * fraction, 1) as distance,\n		round(up_revenue * fraction, 2) as up_revenue,\n		currency_iso,\n		fraction,\n		is_weekend\n	from (\n		select\n			tt.reservation_id,\n			tt.member_id,\n			tt.day,\n			case when total_hours > 0 then tt.hours else 0.0 end as hours,\n			tt.total_distance,\n			tt.up_revenue,\n			tt.currency_iso,\n			case when total_hours > 0 then round(tt.hours * 1.0 / tt.total_hours, 10) else 0 end as fraction,\n			CASE \n				WHEN (date_part(dow, tt.day) = 6 OR date_part(dow, tt.day) = 0) THEN true\n				ELSE FALSE\n			END AS is_weekend\n		from (\n			select \n				tt.member_id,\n				tt.reservation_id,\n				tt.day,\n				tt.total_hours,\n				tt.total_distance,\n				tt.up_revenue,\n				tt.currency_iso,\n				CASE \n					WHEN trunc(start_date) = trunc(end_date) THEN round(datediff(minute, start_date, end_date) / 60.0, 4)\n					WHEN day = trunc(start_date) THEN round(datediff(minute, start_date, day+1) / 60.0, 4)\n					WHEN day = trunc(end_date) THEN round(datediff(minute, day, end_date) / 60.0, 4)\n					ELSE 24\n				END AS hours\n			from (\n				select\n					r.member_id,\n					r.reservation_id, r.start_date, r.end_date,\n					trunc(dateadd(day, seq.num, r.start_date)) as day,\n					r.total_hours,\n					r.total_distance,\n					r.up_revenue,\n					r.currency_iso\n				from month_reservations r \n				join seq\n				on 1=1\n				where trunc(dateadd(day, seq.num, r.start_date)) <= r.end_date\n			) tt\n		) tt\n	)\n);\n\n--drop table if EXISTS reservation_wkday_wkend;\ncreate temp table reservation_wkday_wkend\ndistkey (member_id)\nsortkey (member_id) as (\n	select\n		member_id, currency_iso,\n		sum(wkend_res_count) as wkend_res_count,\n		sum(wkday_res_count) as wkday_res_count,\n		sum(wkend_hours) as wkend_hours,\n		sum(wkday_hours) as wkday_hours,\n		sum(wkend_distance) as wkend_distance,\n		sum(wkday_distance) as wkday_distance,\n		sum(wkend_up_revenue) as wkend_up_revenue,\n		sum(wkday_up_revenue) as wkday_up_revenue\n	from (\n		select\n			member_id, \n			currency_iso,\n			is_weekend,\n			case when is_weekend = true then count(distinct reservation_id) else 0 end as wkend_res_count,\n			case when is_weekend = false then count(distinct reservation_id) else 0 end as wkday_res_count,\n			case when is_weekend = true then sum(hours) else 0 end as wkend_hours,\n			case when is_weekend = false then sum(hours) else 0 end as wkday_hours,\n			case when is_weekend = true then sum(distance) else 0 end as wkend_distance,\n			case when is_weekend = false then sum(distance) else 0 end as wkday_distance,\n			case when is_weekend = true then sum(up_revenue) else 0 end as wkend_up_revenue,\n			case when is_weekend = false then sum(up_revenue) else 0 end as wkday_up_revenue	\n		from reservation_fractions\n		where (day >= {{startDate}} and day < {{runDate}})\n		group by member_id, currency_iso, is_weekend\n	)\n	group by member_id, currency_iso\n);\n\n--drop table if EXISTS member_month_info;\ncreate temp table member_month_info as (\n	select \n		mrt.member_id,\n		mrt.currency_iso,\n		case when (mrc.wkend_res_count > 0 or mrc.wkday_res_count > 0) then false else true end as is_inactive,\n		mrt.total_reservations,\n		mrt.total_hours,\n		mrt.total_distance,\n		mrt.total_up_revenue,\n		mrt.total_late_returns,\n		mrt.trips_completed,\n		rww.wkend_res_count,\n		rww.wkday_res_count,\n		rww.wkend_hours,\n		rww.wkday_hours,\n		rww.wkend_distance,\n		rww.wkday_distance,\n		rww.wkend_up_revenue,\n		rww.wkday_up_revenue\n	from month_reservation_totals mrt\n	left join reservation_wkday_wkend rww\n	on mrt.member_id = rww.member_id\n	and mrt.currency_iso = rww.currency_iso\n	left join (\n		select member_id, \n		sum(wkend_res_count) as wkend_res_count,\n		sum(wkday_res_count) as wkday_res_count\n		from reservation_wkday_wkend\n		group by member_id\n	) mrc\n	on mrt.member_id = mrc.member_id\n);\n",
"marketing.schema", StringHandling.DQUOTE(context.Redshift_Marketing_Schema),
...
"\nINSERT INTO {{marketing.schema}}.s_member_detail_monthly\nWITH person_member AS (\n    SELECT dm.member_id, dm.person_id, dm.current_zipfleet_id\n    FROM {{marketing.schema}}.d_member AS dm\n    WHERE dm.start_date <= {{endDate}}\n    AND dm.end_date >= {{endDate}}\n    AND dm.is_active = TRUE\n)\nSELECT\n	m.person_id,\n	m.member_id,\n	TO_CHAR({{endDate}}::DATE, 'YYYY_MM') as year_month,\n	dateadd(month, -1, {{runDate}})::DATE as start_date,\n	{{endDate}} as end_date,\n	nvl(mmi.currency_iso, 'USD') as currency_iso,\n	m.current_zipfleet_id,\n	nvl(mmi.is_inactive, true) as is_inactive,\n	case when (nvl(mmi.is_inactive, true) = false and mdlm.is_inactive = true) then true else false end as is_reactivation_month,\n	mji.join_date,\n	mji.join_rate_plan_id,\n	rp.segment_class as join_segment_class,\n	mji.join_zipfleet_id,\n	mji.last_join_date,\n	mji.last_join_rate_plan_id,\n	mji.last_join_zipfleet_id,\n	mli.last_left_date,\n	smls.segment as location_segment,\n	sma.metro_id,\n	case when (nvl(mmi.is_inactive, true) = true) then nvl(mdlm.months_inactive, 0) + 1 else 0 end as months_inactive,\n	sma.primary_affiliate_id,\n	sma.primary_plan_category,\n	sma.primary_rate_plan_id,\n	sma.primary_segment_class,\n	srfm.profile as rfm_profile,\n	nvl(mmi.total_reservations, 0) as reservation_count,\n	sma.tenure_category,\n	nvl(mmi.total_distance, 0.0) as total_distance,\n	nvl(mmi.total_hours, 0.0) as total_hours,\n	nvl(mmi.total_late_returns, 0) as total_late_returns,\n	nvl(mmi.total_up_revenue, 0.00) as total_up_revenue,\n	nvl(mmi.trips_completed, 0) as trips_completed,\n	nvl(mmi.wkday_distance, 0.0) as wkday_distance,\n	nvl(mmi.wkday_hours, 0.0) as wkday_hours,\n	nvl(mmi.wkday_res_count, 0) as wkday_res_count,\n	nvl(mmi.wkday_up_revenue, 0.00) as wkday_up_revenue,\n	nvl(mmi.wkend_distance, 0.0) as wkend_distance,\n	nvl(mmi.wkend_hours, 0.0) as wkend_hours,\n	nvl(mmi.wkend_res_count, 0) as wkend_res_count,\n	nvl(mmi.wkend_up_revenue, 0.00) as wkend_up_revenue,\n	TO_CHAR(sm.first_join_date, 'YYYY_MM') as vintage_date\nFROM person_member m\nJOIN {{marketing.schema}}.s_member sm\nON m.member_id = sm.member_id\nLEFT JOIN member_month_info mmi\nON m.member_id = mmi.member_id\nLEFT JOIN member_detail_last_month mdlm\non m.member_id = mdlm.member_id\nLEFT JOIN {{marketing.schema}}.s_member_activity_monthly sma\nON m.member_id = sma.member_id\nAND sma.end_date = {{endDate}}\nLEFT JOIN {{marketing.schema}}.s_member_location_segment_monthly smls\nON m.member_id = smls.member_id\nAND smls.end_date = {{endDate}}\nLEFT JOIN {{marketing.schema}}.s_rfm_monthly srfm\nON m.member_id = srfm.member_id\nAND srfm.end_date = {{endDate}}\nLEFT JOIN {{marketing.schema}}.vw_r_member_join_info mji\nON m.member_id = mji.member_id\nLEFT JOIN {{marketing.schema}}.vw_r_member_left_info mli\nON m.member_id = mli.member_id\nLEFT JOIN {{marketing.schema}}.vw_r_rate_plan rp\nON mji.join_rate_plan_id = rp.rate_plan_id\n;\n",
"marketing.schema", StringHandling.DQUOTE(context.Redshift_Marketing_Schema),
"startDate", DateUtils.dateToSqlStr((Date) globalMap.get("START_DATE")),
"endDate", DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")),
"runDate", DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE"))
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_1", "", Thread.currentThread().getId() + "", "INFO","","Monthly Member details inserted into (s_member_detail_monthly) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_1", 3, "Monthly Member details inserted into (s_member_detail_monthly) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_1_WARN_MESSAGES", "Monthly Member details inserted into (s_member_detail_monthly) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE"))); 
 ...
 */
    ...
    public static void main(String[] args){
        final Process_Monthly_Member_Details Process_Monthly_Member_DetailsClass = new Process_Monthly_Member_Details();

        int exitCode = Process_Monthly_Member_DetailsClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Process_Monthly_Member_Details.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/process_monthly_member_details_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Process_Monthly_Member_Details");
        }
...
/************************************************************************************************
 *     131631 characters generated by Talend Open Studio for Data Integration 
