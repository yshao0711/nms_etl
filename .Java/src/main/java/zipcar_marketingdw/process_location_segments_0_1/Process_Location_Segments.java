...
package zipcar_marketingdw.process_location_segments_0_1;
...
/**
 * Job: Process_Location_Segments Purpose: Computes member location segmentation<br>
 * Description: Computes member location segmentation from reservation data <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Process_Location_Segments implements TalendJob {
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Process_Location_Segments";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_dX5E8Eu2EeeqT_QylVjy4Q", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Process_Location_Segments.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Process_Location_Segments.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
Logging.componentName(jobName, "tWarn_3", "Log_Reset_Location_Segment_Rows");
...
public static class deleted_rowsStruct implements routines.system.IPersistableRow<deleted_rowsStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_Location_Segments = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Process_Location_Segments = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Process_Location_Segments.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Process_Location_Segments.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_Location_Segments = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_Location_Segments = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Process_Location_Segments, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Process_Location_Segments, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_Location_Segments) {
...
"\nDELETE FROM {{marketing.schema}}.s_member_location_segment\nWHERE end_date = {{endDate}}\n",
...
if(deleted_rows != null) { 
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_3", "", Thread.currentThread().getId() + "", "INFO","","Deleted Current Day Location Segment Data for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_3", 3, "Deleted Current Day Location Segment Data for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_3_WARN_MESSAGES", "Deleted Current Day Location Segment Data for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE"))); 
...
"\nCREATE TEMP TABLE account_member AS (\n    SELECT DISTINCT dm.member_id, dm.person_id\n    FROM {{marketing.schema}}.d_member AS dm\n    WHERE dm.start_date <= {{endDate}}\n    AND dm.end_date >= {{endDate}}\n    AND dm.is_active = TRUE\n);\nCREATE TEMP TABLE member_reservation_distance AS\n(\n    SELECT tt.*, nvl(pmm.metro_id, 0) AS metro_id\n    FROM (\n        SELECT \n            sr.reservation_id,\n            sr.member_id,\n            sr.location_id,\n            dl.postal_code,\n            dl.country_iso,\n            mld.distance\n        FROM {{marketing.schema}}.s_reservation sr\n        JOIN {{marketing.schema}}.a_reservation_to_location_distance mld\n        ON sr.member_id = mld.member_id\n        AND sr.location_id = mld.location_id\n		AND sr.is_new_platform = mld.is_location_np\n        AND mld.end_date = '3199-12-31'\n        JOIN (\n            SELECT * FROM {{marketing.schema}}.d_location WHERE end_date='3199-12-31'\n        ) dl\n        ON sr.location_id = dl.location_id\n		AND sr.is_new_platform = dl.is_new_platform\n        WHERE sr.member_id in (select member_id from account_member)\n        AND sr.start_date >= {{startDate}}\n        AND sr.start_date < {{runDate}}\n		AND ((sr.cancel_date is null or sr.cancel_date >= {{runDate}}) and (sr.status is null or sr.status <> 'CANCELLED'))\n    ) tt\n    LEFT JOIN {{marketing.schema}}.l_postalcode_metro_map pmm\n    ON {{marketing.schema}}.f_postalcode_for_mapping(tt.postal_code, tt.country_iso) = pmm.postal_code\n);\nINSERT INTO {{marketing.schema}}.s_member_location_segment\nWITH member_reservation_total AS\n(\n    SELECT member_id, COUNT(reservation_id) AS reservation_total\n    FROM member_reservation_distance\n    GROUP BY member_id\n),\nmember_reservation_at_most_frequent_location AS\n(\n    SELECT member_id, reservation_mfl\n    FROM (\n        SELECT member_id, COUNT(reservation_id) AS reservation_mfl,\n        ROW_NUMBER() OVER (PARTITION BY member_id ORDER BY reservation_mfl DESC) as row_num\n        FROM member_reservation_distance\n        GROUP BY member_id, location_id \n    )\n    WHERE row_num = 1\n),\nmember_reservation_within_median_distance AS (\n    SELECT dist.member_id, COUNT(reservation_id) AS reservation_wmd\n    FROM member_reservation_distance dist\n    JOIN {{marketing.schema}}.l_metro_median_distance mmd\n    ON dist.metro_id = mmd.metro_id\n    WHERE dist.distance <= mmd.distance\n    AND (mmd.start_date < dateadd(year, -1, {{runDate}}) AND mmd.end_date >= dateadd(year, -1, {{runDate}}))\n    GROUP BY dist.member_id\n)\nSELECT\n    person_id,\n    member_id,\n    proximity,\n    uniformity,\n    CASE\n        WHEN proximity = 0 and uniformity = 0 THEN 'No Res'\n        WHEN proximity < lsq.proximity_pct and uniformity < lsq.uniformity_pct THEN 'Traveler'\n        WHEN proximity < lsq.proximity_pct and uniformity >= lsq.uniformity_pct THEN 'Intermittent'\n        WHEN proximity >= lsq.proximity_pct and uniformity < lsq.uniformity_pct THEN 'Opportunists'\n        WHEN proximity >= lsq.proximity_pct and uniformity >= lsq.uniformity_pct THEN 'HomeBody'\n        ELSE ''\n    END AS segment,\n    {{startDate}} AS start_date,\n    {{endDate}} AS end_date\nFROM (\n    SELECT \n        am.person_id, \n        am.member_id,\n        NVL(ROUND(mrwmd.reservation_wmd * 100.0 / mrt.reservation_total), 0) AS proximity,\n        NVL(ROUND(mrmfl.reservation_mfl * 100.0 / mrt.reservation_total), 0) AS uniformity\n    FROM account_member am\n    LEFT JOIN member_reservation_total mrt\n    ON am.member_id = mrt.member_id\n    LEFT JOIN member_reservation_at_most_frequent_location mrmfl\n    ON am.member_id = mrmfl.member_id\n    LEFT JOIN member_reservation_within_median_distance mrwmd\n    ON am.member_id = mrwmd.member_id\n) tt\nJOIN {{marketing.schema}}.l_location_segment_quadrant lsq\nON 1=1 AND lsq.end_date='3199-12-31'\n",
"marketing.schema", StringHandling.DQUOTE(context.Redshift_Marketing_Schema),
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_6", "", Thread.currentThread().getId() + "", "INFO","","Member location segment data inserted into (s_member_location_segment) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_6", 3, "Member location segment data inserted into (s_member_location_segment) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_6_WARN_MESSAGES", "Member location segment data inserted into (s_member_location_segment) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE"))); 
 ...
 */
    ...
    public static void main(String[] args){
        final Process_Location_Segments Process_Location_SegmentsClass = new Process_Location_Segments();

        int exitCode = Process_Location_SegmentsClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Process_Location_Segments.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/process_location_segments_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Process_Location_Segments");
        }
...
/************************************************************************************************
 *     130312 characters generated by Talend Open Studio for Data Integration 
