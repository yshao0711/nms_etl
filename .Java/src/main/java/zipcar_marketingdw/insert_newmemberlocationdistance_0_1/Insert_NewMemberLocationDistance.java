...
package zipcar_marketingdw.insert_newmemberlocationdistance_0_1;
...
/**
 * Job: Insert_NewMemberLocationDistance Purpose: Save new records<br>
 * Description:  <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Insert_NewMemberLocationDistance implements TalendJob {
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Insert_NewMemberLocationDistance";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_ut-4cFa2EeeQ1qlL72Siww", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Insert_NewMemberLocationDistance.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Insert_NewMemberLocationDistance.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
Logging.componentName(jobName, "tWarn_2", "Log_NewMemberLocationDistance");
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Insert_NewMemberLocationDistance = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Insert_NewMemberLocationDistance = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Insert_NewMemberLocationDistance.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Insert_NewMemberLocationDistance.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Insert_NewMemberLocationDistance = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Insert_NewMemberLocationDistance = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Insert_NewMemberLocationDistance, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Insert_NewMemberLocationDistance, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Insert_NewMemberLocationDistance) {
...
"\nINSERT INTO {{marketing.schema}}.a_reservation_to_location_distance\nWITH member_address AS (\n	SELECT a1.member_id, a1.latitude, a1.longitude\n	FROM {{marketing.schema}}.r_address a1\n	LEFT JOIN {{marketing.schema}}.r_address a2\n	ON a1.address_id = a2.address_id   \n	AND a1.imported_date < a2.imported_date\n	WHERE a2.imported_date IS NULL\n	AND a1.member_id IS NOT NULL\n	AND a1.address_type != 'billing'\n	AND (a1.latitude IS NOT NULL and a1.longitude IS NOT NULL)\n	AND (a1.deleted_date IS NULL OR a1.deleted_date > {{runDate}})\n),\nnew_member_location AS (\n	SELECT\n		dm.person_id,\n		sr.member_id, \n		sr.location_id,\n		dl.is_new_platform\n	FROM {{marketing.schema}}.s_reservation sr\n    JOIN (\n    	SELECT person_id, member_id from {{marketing.schema}}.d_member WHERE end_date='3199-12-31'\n    ) dm\n    on sr.member_id = dm.member_id\n    JOIN (\n	    SELECT * FROM {{marketing.schema}}.d_location WHERE end_date='3199-12-31'\n	) dl\n	ON sr.location_id = dl.location_id\n	AND sr.is_new_platform = dl.is_new_platform\n	JOIN member_address ma\n	ON sr.member_id = ma.member_id\n	WHERE sr.start_date >= '2015-01-01' \n	AND sr.start_date <= {{runDate}}\n	AND ((sr.cancel_date is null or sr.cancel_date >= {{runDate}}) and (sr.status is null or sr.status <> 'CANCELLED'))\n	EXCEPT (SELECT person_id, member_id, location_id, is_location_np FROM {{marketing.schema}}.a_reservation_to_location_distance WHERE end_date='3199-12-31')\n)\nSELECT \n	nml.person_id,\n	nml.member_id,\n	nml.location_id,\n	nml.is_new_platform as is_location_np,\n	MIN({{marketing.schema}}.f_distance_miles(dl.latitude, dl.longitude, ma.latitude, ma.longitude)) as distance,\n	{{runDate}} AS start_date,\n	'3199-12-31' as end_date\nFROM new_member_location nml\nJOIN (\n    SELECT * FROM {{marketing.schema}}.d_location WHERE end_date='3199-12-31'\n) dl\nON nml.location_id = dl.location_id\nAND nml.is_new_platform = dl.is_new_platform\nJOIN member_address ma\nON nml.member_id = ma.member_id\nGROUP BY nml.person_id, nml.member_id, nml.location_id, nml.is_new_platform\n",
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_2", "", Thread.currentThread().getId() + "", "INFO","","New member location distance added.","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_2", 3, "New member location distance added.", 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_2_WARN_MESSAGES", "New member location distance added."); 
 ...
 */
    ...
    public static void main(String[] args){
        final Insert_NewMemberLocationDistance Insert_NewMemberLocationDistanceClass = new Insert_NewMemberLocationDistance();

        int exitCode = Insert_NewMemberLocationDistanceClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Insert_NewMemberLocationDistance.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/insert_newmemberlocationdistance_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Insert_NewMemberLocationDistance");
        }
...
/************************************************************************************************
 *     110621 characters generated by Talend Open Studio for Data Integration 
