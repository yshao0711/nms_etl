...
package zipcar_marketingdw.update_memberphoneinfo_0_1;
...
/**
 * Job: Update_MemberPhoneInfo Purpose: Updates the members phone fields<br>
 * Description:  <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Update_MemberPhoneInfo implements TalendJob {
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Update_MemberPhoneInfo";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_0bSKYIm3Eee5n-XeCGSojw", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Update_MemberPhoneInfo.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Update_MemberPhoneInfo.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
			...
			public void tRedshiftRow_8_error(Exception exception, String errorComponent, final java.util.Map<String, Object> globalMap) throws TalendException {
				
				end_Hash.put(errorComponent, System.currentTimeMillis());
				
				status = "failure";
				
					tRedshiftRow_8_onSubJobError(exception, errorComponent, globalMap);
			}
...
resumeUtil.addLog("SYSTEM_LOG", "NODE:"+ errorComponent, "", Thread.currentThread().getId()+ "", "FATAL", "", exception.getMessage(), ResumeUtil.getExceptionStackTrace(exception),"");
			...
			public void tRedshiftRow_8_onSubJobError(Exception exception, String errorComponent, final java.util.Map<String, Object> globalMap) throws TalendException {
...
Logging.componentName(jobName, "tWarn_2", "Log_CalcMemberPhone");
Logging.componentName(jobName, "tWarn_3", "Log_CalcUpdatedMembers");
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Update_MemberPhoneInfo = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberPhoneInfo = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberPhoneInfo.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberPhoneInfo.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberPhoneInfo = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberPhoneInfo = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberPhoneInfo, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberPhoneInfo, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Update_MemberPhoneInfo) {
...
public void tRedshiftRow_5Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	...
	boolean whetherReject_tRedshiftRow_5 = false;
				conn_tRedshiftRow_5 = (java.sql.Connection)globalMap.get("conn_tRedshiftConnection_2");
...
"\nCREATE TEMP TABLE current_members AS\n    SELECT\n        rm.member_id,\n        CASE \n            WHEN len(rm.sms_number)>15 THEN NULL\n            ELSE rm.sms_number\n        END AS sms_number,\n        rm.preferred_address_id,\n        rm.mobile_promo_p\n    FROM {{marketing.schema}}.vw_r_member rm\n    WHERE \n        rm.member_id in (\n            SELECT DISTINCT member_id FROM {{marketing.schema}}.d_member WHERE end_date='3199-12-31'\n        )\n;\nCREATE TEMP TABLE member_phones AS\n    select cm.member_id, \n        ra.address_id, ra.address_type, \n        CASE\n            WHEN len(ra.phone_number_one)>15 THEN NULL\n            ELSE ra.phone_number_one\n        END AS phone_number_one,\n        CASE\n            WHEN len(ra.phone_number_two)>15 THEN NULL\n            ELSE ra.phone_number_two\n        END AS phone_number_two,\n        ra.creation_date, ra.last_modified\n    from current_members cm\n    join {{marketing.schema}}.vw_r_address ra\n    on cm.member_id = ra.member_id\n    and ra.creation_date <= {{current.date}}\n    and (ra.phone_number_one is not null or ra.phone_number_two is not null)\n    and (len(ra.phone_number_one) <= 15 or len(ra.phone_number_two) <= 15)\n    and (ra.deleted_date is null or ra.deleted_date > {{current.date}})\n;\nCREATE TEMP TABLE member_billing_phone AS\n    select * from (\n        select member_id, phone_number_one, phone_number_two,\n        row_number() OVER (PARTITION by member_id order by last_modified desc, creation_date desc) row_num\n        from member_phones\n        where address_type='billing'\n    ) where row_num=1\n;\nCREATE TEMP TABLE member_home_phone AS\n    select * from (\n        select member_id, phone_number_one, phone_number_two,\n        row_number() OVER (PARTITION by member_id order by last_modified desc, creation_date desc) row_num\n        from member_phones\n        where address_type='home'\n    ) where row_num=1\n;\nCREATE TEMP TABLE member_work_phone AS\n    select * from (\n        select member_id, phone_number_one, phone_number_two,\n        row_number() OVER (PARTITION by member_id order by last_modified desc, creation_date desc) row_num\n        from member_phones\n        where address_type='work'\n    ) where row_num=1\n;\nCREATE TEMP TABLE member_other_phone AS\n    select * from (\n        select member_id, phone_number_one, phone_number_two,\n        row_number() OVER (PARTITION by member_id order by last_modified desc, creation_date desc) row_num\n        from member_phones\n        where address_type='other'\n    ) where row_num=1\n;\nCREATE TEMP TABLE preferred_address_phone AS\n    select mp.member_id, phone_number_one, phone_number_two\n    from member_phones mp\n    join current_members cm\n    on mp.member_id=cm.member_id\n    and mp.address_id=cm.preferred_address_id\n;\nCREATE TEMP TABLE calc_member_phone AS \nselect cm.member_id,\n    COALESCE(cm.sms_number, \n        pa.phone_number_one, pa.phone_number_two,\n        bp.phone_number_one, bp.phone_number_two, \n        hp.phone_number_one, hp.phone_number_two,\n        wp.phone_number_one, wp.phone_number_two,\n        op.phone_number_one, op.phone_number_two\n    ) as phone_main,\n    CASE\n        WHEN cm.sms_number is not null THEN TRUE\n        ELSE FALSE\n    END AS is_mobile_phone,\n    CASE\n        WHEN cm.mobile_promo_p = 't' THEN TRUE\n        ELSE FALSE\n    END AS is_textable\nfrom current_members cm\nleft join preferred_address_phone pa\non cm.member_id = pa.member_id\nleft join member_billing_phone bp\non cm.member_id=bp.member_id\nleft join member_home_phone hp\non cm.member_id=hp.member_id\nleft join member_work_phone wp\non cm.member_id=wp.member_id\nleft join member_other_phone op\non cm.member_id=op.member_id;\n",
"marketing.schema", context.Redshift_Marketing_Schema,
"current.date", DateUtils.dateToSqlStr((Date) globalMap.get("CURRENT_DATE"))
);
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_2", "", Thread.currentThread().getId() + "", "INFO","","Calculated Current Member Phone Fields","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_2", 3, "Calculated Current Member Phone Fields", 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_2_WARN_MESSAGES", "Calculated Current Member Phone Fields"); 
...
"\nCREATE TEMP TABLE calc_updated_members AS \nWITH current_members AS (\n	SELECT * FROM {{marketing.schema}}.d_member\n	WHERE end_date = '3199-12-31'\n)\nSELECT dm.* \nFROM current_members dm \nJOIN calc_member_phone cmp\nON dm.member_id = cmp.member_id\nAND (\n	(dm.phone_main IS NULL OR dm.phone_main <> cmp.phone_main)\n	OR (dm.is_mobile_phone IS NULL OR dm.is_mobile_phone <> cmp.is_mobile_phone)\n	OR (dm.is_textable IS NULL OR dm.is_textable <> cmp.is_textable)\n)\n",
"marketing.schema", context.Redshift_Marketing_Schema
);
...
public void tRedshiftRow_7Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	...
	boolean whetherReject_tRedshiftRow_7 = false;
				conn_tRedshiftRow_7 = (java.sql.Connection)globalMap.get("conn_tRedshiftConnection_2");
...
"\nUPDATE calc_updated_members  SET\n    phone_main = cmp.phone_main,\n	is_mobile_phone = cmp.is_mobile_phone,\n	is_textable = cmp.is_textable,\n    start_date = {{current.date}},\n    imported_date = {{current.date}}\nFROM calc_member_phone cmp\nWHERE\n     calc_updated_members.member_id=cmp.member_id\n",
"marketing.schema", context.Redshift_Marketing_Schema,
"current.date", DateUtils.dateToSqlStr((Date) globalMap.get("CURRENT_DATE"))
);
 ...
 */
							...
							tRedshiftRow_8Process(globalMap); 
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_3", "", Thread.currentThread().getId() + "", "INFO","","Calculated Updated Member Records","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_3", 3, "Calculated Updated Member Records", 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_3_WARN_MESSAGES", "Calculated Updated Member Records"); 
...
public void tRedshiftRow_8Process(final java.util.Map<String, Object> globalMap) throws TalendException {
	globalMap.put("tRedshiftRow_8_SUBPROCESS_STATE", 0);
 ...
 final boolean execStat = this.execStat;
	 ...
	 */
		...
		ok_Hash.put("tRedshiftRow_8", false);
		start_Hash.put("tRedshiftRow_8", System.currentTimeMillis());
		
	
	currentComponent="tRedshiftRow_8";

	
		int tos_count_tRedshiftRow_8 = 0;
		
    	class BytesLimit65535_tRedshiftRow_8{
    		public void limitLog4jByte() throws Exception{
    			
    		}
    	}
    	
        new BytesLimit65535_tRedshiftRow_8().limitLog4jByte();

	java.sql.Connection conn_tRedshiftRow_8 = null;
	String query_tRedshiftRow_8 = "";
	boolean whetherReject_tRedshiftRow_8 = false;
				conn_tRedshiftRow_8 = (java.sql.Connection)globalMap.get("conn_tRedshiftConnection_2");
			
		java.sql.Statement stmt_tRedshiftRow_8 = conn_tRedshiftRow_8.createStatement();
...
/**
 * [tRedshiftRow_8 begin ] stop
 */
	
	/**
	 * [tRedshiftRow_8 main ] start
	 */
...
query_tRedshiftRow_8 = StringUtils.templateString(
"\nUPDATE {{marketing.schema}}.d_member SET\n	phone_main = calc.phone_main,\n	is_mobile_phone = calc.is_mobile_phone,\n	is_textable = calc.is_textable\nFROM calc_updated_members calc\nWHERE\n	d_member.member_id=calc.member_id\n	AND (\n		(d_member.phone_main IS NULL OR d_member.phone_main  <>  calc.phone_main)\n		OR (d_member.is_mobile_phone IS NULL OR d_member.is_mobile_phone  <>  calc.is_mobile_phone)\n		OR (d_member.is_textable IS NULL OR d_member.is_textable  <>  calc.is_textable)\n	)\n	AND d_member.imported_date = {{current.date}}\n	AND d_member.end_date='3199-12-31'\n",
"marketing.schema", context.Redshift_Marketing_Schema,
"current.date", DateUtils.dateToSqlStr((Date) globalMap.get("CURRENT_DATE"))
);
whetherReject_tRedshiftRow_8 = false;
globalMap.put("tRedshiftRow_8_QUERY",query_tRedshiftRow_8);
try {
		stmt_tRedshiftRow_8.execute(query_tRedshiftRow_8);
		
	} catch (java.lang.Exception e) {
		whetherReject_tRedshiftRow_8 = true;
		
			throw(e);
			
	}
	
	if(!whetherReject_tRedshiftRow_8) {
	...
	tos_count_tRedshiftRow_8++;

/**
 * [tRedshiftRow_8 main ] stop
 */
	
	/**
	 * [tRedshiftRow_8 end ] start
	 */
	...
	stmt_tRedshiftRow_8.close();	

 

ok_Hash.put("tRedshiftRow_8", true);
end_Hash.put("tRedshiftRow_8", System.currentTimeMillis());




/**
 * [tRedshiftRow_8 end ] stop
 */
				}//end the resume

				
				    			if(resumeEntryMethodName == null || globalResumeTicket){
				    				resumeUtil.addLog("CHECKPOINT", "CONNECTION:SUBJOB_OK:tRedshiftRow_8:OnSubjobOk", "", Thread.currentThread().getId() + "", "", "", "", "", "");
								}	    				    			
 ...
 */
		...
		globalMap.put("tRedshiftRow_8_SUBPROCESS_STATE", 1);
	}
...
"\nINSERT INTO {{marketing.schema}}.d_member\n    SELECT DISTINCT calc.* FROM calc_updated_members calc\n    JOIN {{marketing.schema}}.d_member dm ON\n        dm.end_date = '3199-12-31'\n        AND dm.member_id = calc.member_id\n        AND (\n			(dm.phone_main IS NULL OR dm.phone_main  <>  calc.phone_main)\n			OR (dm.is_mobile_phone IS NULL OR dm.is_mobile_phone  <>  calc.is_mobile_phone)\n			OR (dm.is_textable IS NULL OR dm.is_textable  <>  calc.is_textable)\n		)\n	    AND dm.imported_date < {{current.date}}\n;\nUPDATE {{marketing.schema}}.d_member SET\n    end_date={{inactive.date}}\nFROM calc_updated_members calc\nWHERE\n    d_member.end_date = '3199-12-31'\n    AND d_member.member_id = calc.member_id\n    AND (\n		(d_member.phone_main IS NULL OR d_member.phone_main  <>  calc.phone_main)\n		OR (d_member.is_mobile_phone IS NULL OR d_member.is_mobile_phone  <>  calc.is_mobile_phone)\n		OR (d_member.is_textable IS NULL OR d_member.is_textable  <>  calc.is_textable)\n	)\n	AND d_member.imported_date < {{current.date}}\n",
"marketing.schema", context.Redshift_Marketing_Schema,
"current.date", DateUtils.dateToSqlStr((Date) globalMap.get("CURRENT_DATE")),
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_1", "", Thread.currentThread().getId() + "", "INFO","","Members Updated Phone Fields","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_1", 3, "Members Updated Phone Fields", 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_1_WARN_MESSAGES", "Members Updated Phone Fields"); 
 ...
 */
    ...
    public static void main(String[] args){
        final Update_MemberPhoneInfo Update_MemberPhoneInfoClass = new Update_MemberPhoneInfo();

        int exitCode = Update_MemberPhoneInfoClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Update_MemberPhoneInfo.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/update_memberphoneinfo_0_1/contexts/"+contextStr+".properties");
...
try {
errorCode = null;tRedshiftRow_5Process(globalMap);
if(!"failure".equals(status)) { status = "end"; }
}catch (TalendException e_tRedshiftRow_5) {
globalMap.put("tRedshiftRow_5_SUBPROCESS_STATE", -1);

e_tRedshiftRow_5.printStackTrace();
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Update_MemberPhoneInfo");
        }
...
/************************************************************************************************
 *     137568 characters generated by Talend Open Studio for Data Integration 
