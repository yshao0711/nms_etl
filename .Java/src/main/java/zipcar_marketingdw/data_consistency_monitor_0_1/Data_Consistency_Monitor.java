...
package zipcar_marketingdw.data_consistency_monitor_0_1;
...
/**
 * Job: Data_Consistency_Monitor Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status 
 */
public class Data_Consistency_Monitor implements TalendJob {
	...
	public class ContextProperties extends PropertiesWithType {
		...
		public void synchronizeContext(){
			...
			if(sendMailCc_SchemaMonitor != null){
				
					this.setProperty("sendMailCc_SchemaMonitor", sendMailCc_SchemaMonitor.toString());
...
public String sendMailCc_SchemaMonitor;
public String getSendMailCc_SchemaMonitor(){
	return this.sendMailCc_SchemaMonitor;
}
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Data_Consistency_Monitor";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_8XBR8CekEeikmpP71yGSvg", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Data_Consistency_Monitor.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Data_Consistency_Monitor.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Data_Consistency_Monitor = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Data_Consistency_Monitor = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Data_Consistency_Monitor.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Data_Consistency_Monitor.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Data_Consistency_Monitor = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Data_Consistency_Monitor = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Data_Consistency_Monitor, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Data_Consistency_Monitor, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Data_Consistency_Monitor) {
...
globalMap.put("SharedMap", new HashMap<String, Object>());
globalMap.put("EmailContent", new StringBuilder());
 ...
 */
  ...
  try{
        ...
        if(key_tContextLoad_1!=null && "sendMailCc_BulkLoad".equals(key_tContextLoad_1))
        {
           context.sendMailCc_BulkLoad=value_tContextLoad_1;
        }

        if(key_tContextLoad_1!=null && "sendMailCc_IncompleteApp".equals(key_tContextLoad_1))
        {
           context.sendMailCc_IncompleteApp=value_tContextLoad_1;
        }

        if(key_tContextLoad_1!=null && "sendMailCc_RatePlanMapping".equals(key_tContextLoad_1))
        {
           context.sendMailCc_RatePlanMapping=value_tContextLoad_1;
        }

        if(key_tContextLoad_1!=null && "sendMailCc_SchemaMonitor".equals(key_tContextLoad_1))
        {
           context.sendMailCc_SchemaMonitor=value_tContextLoad_1;
        }
 ...
 */
	...
	java.util.Map<String, Object> parentContextMap_tRunJob_1 = new java.util.HashMap<String, Object>();
		...
		context.synchronizeContext();
			...
			parentContextMap_tRunJob_1.put("sendMailCc_BulkLoad", context.sendMailCc_BulkLoad);
			paraList_tRunJob_1.add("--context_type " + "sendMailCc_BulkLoad" + "=" + "id_String");
		
			parentContextMap_tRunJob_1.put("sendMailCc_IncompleteApp", context.sendMailCc_IncompleteApp);
			paraList_tRunJob_1.add("--context_type " + "sendMailCc_IncompleteApp" + "=" + "id_String");
			...
			parentContextMap_tRunJob_1.put("sendMailCc_SchemaMonitor", context.sendMailCc_SchemaMonitor);
			paraList_tRunJob_1.add("--context_type " + "sendMailCc_SchemaMonitor" + "=" + "id_String");
	...
	Object obj_tRunJob_1 = null;
		...
		zipcar_marketingdw.task_monitor_suspended_members_0_1.Task_Monitor_Suspended_Members childJob_tRunJob_1 = new zipcar_marketingdw.task_monitor_suspended_members_0_1.Task_Monitor_Suspended_Members();
	    // pass DataSources
...
Map<String, Object> sharedMap = (Map<String, Object>)globalMap.get("SharedMap");
if(sharedMap.get("members") == null) return;
if(Integer.parseInt((String)(sharedMap.get("members"))) == 0) return;

StringBuilder emailContent = (StringBuilder) globalMap.get("EmailContent");
StringBuilder sbQuery = new StringBuilder();
sbQuery.append("select \n");
sbQuery.append("	dm.member_id, dm.is_active, dm.start_date, dm.end_date,\n");
sbQuery.append("	rm.current_status,\n");
sbQuery.append("	sr.reservation_id, sr.creation_date as res_creation_date, \n");
sbQuery.append("	sr.start_date as res_start_date, sr.end_date as res_end_date, \n");
sbQuery.append("	sr.cancel_date as res_cancel_date\n");
sbQuery.append("from {{marketing.schema}}.d_member dm\n");
sbQuery.append("join {{marketing.schema}}.vw_r_member rm\n");
sbQuery.append("on dm.member_id = rm.member_id\n");
sbQuery.append("and rm.current_status = 'suspended'\n");
sbQuery.append("join {{marketing.schema}}.s_reservation sr\n");
sbQuery.append("on dm.member_id = sr.member_id\n");
sbQuery.append("and (sr.cancel_date is null OR sr.cancel_date >= CURRENT_DATE)\n");
sbQuery.append("and (sr.end_date >= CURRENT_DATE)\n");
sbQuery.append("where dm.end_date = '3199-12-31'\n");
sbQuery.append("and sr.creation_date > dm.start_date -- reservation made after suspended\n");
sbQuery.append("order by dm.member_id, sr.end_date\n");
sbQuery.append(";\n");
String query = sbQuery.toString().replaceAll("\\{\\{marketing.schema\\}\\}", context.Redshift_Marketing_Schema);

emailContent.append("<h3>Suspended members with reservations</h3>\n");
emailContent.append("<p>There is a total of<b> " + sharedMap.get("members") + " suspended members</b> that have total of<b> " + sharedMap.get("reservations") + " reservations</b>.</p>\n");
emailContent.append("<p>To view the detail information, please run the following SQL query: </p>\n");
emailContent.append("<pre><code>" + query + "</code></pre>\n");
 ...
 */
	...
	java.util.Map<String, Object> parentContextMap_tRunJob_2 = new java.util.HashMap<String, Object>();
		...
		context.synchronizeContext();
			...
			parentContextMap_tRunJob_2.put("sendMailCc_BulkLoad", context.sendMailCc_BulkLoad);
			paraList_tRunJob_2.add("--context_type " + "sendMailCc_BulkLoad" + "=" + "id_String");
		
			parentContextMap_tRunJob_2.put("sendMailCc_IncompleteApp", context.sendMailCc_IncompleteApp);
			paraList_tRunJob_2.add("--context_type " + "sendMailCc_IncompleteApp" + "=" + "id_String");
		
			parentContextMap_tRunJob_2.put("sendMailCc_RatePlanMapping", context.sendMailCc_RatePlanMapping);
			paraList_tRunJob_2.add("--context_type " + "sendMailCc_RatePlanMapping" + "=" + "id_String");
		
			parentContextMap_tRunJob_2.put("sendMailCc_SchemaMonitor", context.sendMailCc_SchemaMonitor);
			paraList_tRunJob_2.add("--context_type " + "sendMailCc_SchemaMonitor" + "=" + "id_String");
	...
	Object obj_tRunJob_2 = null;
		...
		obj_tRunJob_2 = (Map)globalMap.get("SharedMap");
		if(obj_tRunJob_2!=null) {
			paraList_tRunJob_2.add("--context_param sharedMap=" + RuntimeUtils.tRunJobConvertContext(obj_tRunJob_2));
		} else {
			paraList_tRunJob_2.add("--context_param sharedMap=" + NULL_VALUE_EXPRESSION_IN_COMMAND_STRING_FOR_CHILD_JOB_ONLY);
		}
		
		parentContextMap_tRunJob_2.put("sharedMap", obj_tRunJob_2);
	
	
		zipcar_marketingdw.task_monitor_unmapped_postal_codes_0_1.Task_Monitor_Unmapped_Postal_Codes childJob_tRunJob_2 = new zipcar_marketingdw.task_monitor_unmapped_postal_codes_0_1.Task_Monitor_Unmapped_Postal_Codes();
	    // pass DataSources
...
Map<String, Object> sharedMap = (Map<String, Object>)globalMap.get("SharedMap");
if(sharedMap.get("total_unmapped_postal_codes_with_reservations") == null) return;
if(Integer.parseInt((String)(sharedMap.get("total_unmapped_postal_codes_with_reservations"))) == 0) return;

StringBuilder emailContent = (StringBuilder) globalMap.get("EmailContent");
StringBuilder sbQuery = new StringBuilder();
sbQuery.append("with location_no_metro as\n");
sbQuery.append("(\n");
sbQuery.append("	select\n");
sbQuery.append("	dl.location_id, dl.is_new_platform, dl.postal_code, dl.country_iso, mpm.metro_id\n");
sbQuery.append("	from {{marketing.schema}}.d_location dl\n");
sbQuery.append("	left join (\n");
sbQuery.append("		select mtr.metro_id, pmm.postal_code, mtr.country_iso\n");
sbQuery.append("		from {{marketing.schema}}.l_metro mtr\n");
sbQuery.append("		join {{marketing.schema}}.l_postalcode_metro_map pmm\n");
sbQuery.append("		on pmm.metro_id = mtr.metro_id\n");
sbQuery.append("		where mtr.end_date = '3199-12-31'\n");
sbQuery.append("		and pmm.end_date = '3199-12-31'\n");
sbQuery.append("	) mpm\n");
sbQuery.append("	on {{marketing.schema}}.f_postalcode_for_mapping(dl.postal_code, dl.country_iso) = mpm.postal_code\n");
sbQuery.append("	and dl.country_iso = mpm.country_iso\n");
sbQuery.append("	where dl.end_date='3199-12-31'\n");
sbQuery.append("	and dl.is_active = true\n");
sbQuery.append("	and dl.country_iso in ('US', 'CA')\n");
sbQuery.append("	and mpm.metro_id is null\n");
sbQuery.append("),\n");
sbQuery.append("postal_code_reservations as (\n");
sbQuery.append("	select lnm.postal_code, rp.segment_class, count(*) as reservation_count\n");
sbQuery.append("	from {{marketing.schema}}.s_reservation sr\n");
sbQuery.append("	join location_no_metro lnm\n");
sbQuery.append("	on sr.location_id = lnm.location_id\n");
sbQuery.append("	and sr.is_new_platform = lnm.is_new_platform\n");
sbQuery.append("	left join {{marketing.schema}}.vw_r_rate_plan rp\n");
sbQuery.append("	on sr.rate_plan_id = rp.rate_plan_id\n");
sbQuery.append("	where sr.start_date <= current_date\n");
sbQuery.append("	and ((sr.cancel_date is null or sr.cancel_date > current_date) and (sr.status is null or sr.status <> 'CANCELLED'))\n");
sbQuery.append("	group by lnm.postal_code, rp.segment_class	\n");
sbQuery.append(")\n");
sbQuery.append("select \n");
sbQuery.append("	t2.total as total_unmapped_postal_codes, \n");
sbQuery.append("	count(DISTINCT postal_code) as total_unmapped_postal_codes_with_reservations, \n");
sbQuery.append("	sum(reservation_count) as total_unmapped_reservations, \n");
sbQuery.append("	sum(reservation_count)/count(DISTINCT postal_code) as average_unmapped_reservations,\n");
sbQuery.append("	t3.total_reservations_business, t3.average_reservations_business,\n");
sbQuery.append("	t4.total_reservations_collegiate, t4.average_reservations_collegiate,\n");
sbQuery.append("	t5.total_reservations_consumer, t5.average_reservations_consumer\n");
sbQuery.append("from postal_code_reservations t1\n");
sbQuery.append("join (select count(*) as total from (select postal_code from location_no_metro group by postal_code)) t2\n");
sbQuery.append("on 1=1\n");
sbQuery.append("join (select sum(reservation_count) as total_reservations_business, avg(reservation_count) as average_reservations_business from postal_code_reservations where segment_class = 'Business') t3\n");
sbQuery.append("on 1=1\n");
sbQuery.append("join (select sum(reservation_count) as total_reservations_collegiate, avg(reservation_count) as average_reservations_collegiate from postal_code_reservations where segment_class = 'Collegiate') t4\n");
sbQuery.append("on 1=1\n");
sbQuery.append("join (select sum(reservation_count) as total_reservations_consumer, avg(reservation_count) as average_reservations_consumer from postal_code_reservations where segment_class = 'Consumer') t5\n");
sbQuery.append("on 1=1\n");
sbQuery.append("group by t2.total, t3.total_reservations_business, t3.average_reservations_business, t4.total_reservations_collegiate, t4.average_reservations_collegiate, t5.total_reservations_consumer, t5.average_reservations_consumer\n");
sbQuery.append(";\n");
String query = sbQuery.toString().replaceAll("\\{\\{marketing.schema\\}\\}", context.Redshift_Marketing_Schema);

emailContent.append("<h3>Unmapped Postal Codes</h3>\n");
emailContent.append("<p>There is a total of<b> " + sharedMap.get("total_unmapped_postal_codes") + "</b> unmapped postal codes, <b>" + sharedMap.get("total_unmapped_postal_codes_with_reservations") + "</b> of them have reservations containing the following statistics:</p>\n");
emailContent.append("<table><tr>\n");
emailContent.append("<td>Total Postal Codes</td><td>Total Reservations</td><td>Sub Total Business</td><td>Sub Total Collegiate</td><td>Sub Total Consumer</td>\n");
emailContent.append("</tr>\n");
emailContent.append("<tr>\n");
emailContent.append("<td>" + sharedMap.get("total_unmapped_postal_codes_with_reservations") + "</td>\n");
emailContent.append("<td>" + sharedMap.get("total_unmapped_reservations") + "</td>\n");
emailContent.append("<td>" + sharedMap.get("total_reservations_business") + "</td>\n");
emailContent.append("<td>" + sharedMap.get("total_reservations_collegiate") + "</td>\n");
emailContent.append("<td>" + sharedMap.get("total_reservations_consumer") + "</td>\n");
emailContent.append("</tr></table>\n");
emailContent.append("<p>\n");
emailContent.append("The average number of reservations per unmapped postal code: " + sharedMap.get("average_unmapped_reservations") + "<br>\n");
emailContent.append("The average number of reservations with business rate plans: " + sharedMap.get("average_reservations_business") + "<br>\n");
emailContent.append("The average number of reservations with collegiate rate plans: " + sharedMap.get("average_reservations_collegiate") + "<br>\n");
emailContent.append("The average number of reservations with consumer rate plans: " + sharedMap.get("average_reservations_consumer") + "<br>\n");
 ...
 */
	...
	java.util.Map<String, Object> parentContextMap_tRunJob_3 = new java.util.HashMap<String, Object>();
		...
		context.synchronizeContext();
			...
			parentContextMap_tRunJob_3.put("sendMailCc_BulkLoad", context.sendMailCc_BulkLoad);
			paraList_tRunJob_3.add("--context_type " + "sendMailCc_BulkLoad" + "=" + "id_String");
		
			parentContextMap_tRunJob_3.put("sendMailCc_IncompleteApp", context.sendMailCc_IncompleteApp);
			paraList_tRunJob_3.add("--context_type " + "sendMailCc_IncompleteApp" + "=" + "id_String");
		
			parentContextMap_tRunJob_3.put("sendMailCc_RatePlanMapping", context.sendMailCc_RatePlanMapping);
			paraList_tRunJob_3.add("--context_type " + "sendMailCc_RatePlanMapping" + "=" + "id_String");
		
			parentContextMap_tRunJob_3.put("sendMailCc_SchemaMonitor", context.sendMailCc_SchemaMonitor);
			paraList_tRunJob_3.add("--context_type " + "sendMailCc_SchemaMonitor" + "=" + "id_String");
	...
	Object obj_tRunJob_3 = null;
		...
		obj_tRunJob_3 = (Map)globalMap.get("SharedMap");
		if(obj_tRunJob_3!=null) {
			paraList_tRunJob_3.add("--context_param sharedMap=" + RuntimeUtils.tRunJobConvertContext(obj_tRunJob_3));
		} else {
			paraList_tRunJob_3.add("--context_param sharedMap=" + NULL_VALUE_EXPRESSION_IN_COMMAND_STRING_FOR_CHILD_JOB_ONLY);
		}
		
		parentContextMap_tRunJob_3.put("sharedMap", obj_tRunJob_3);
	
	
		zipcar_marketingdw.task_monitor_reservation_counts_0_1.Task_Monitor_Reservation_Counts childJob_tRunJob_3 = new zipcar_marketingdw.task_monitor_reservation_counts_0_1.Task_Monitor_Reservation_Counts();
	    // pass DataSources
...
///setting global variables to pass back to parent job
Map<String, Object> sharedMap = (Map<String, Object>)globalMap.get("SharedMap");
if(sharedMap.get("members_with_mismatched_reservations") == null) return;
if(Integer.parseInt((String)(sharedMap.get("members_with_mismatched_reservations"))) == 0) return;

StringBuilder emailContent = (StringBuilder) globalMap.get("EmailContent");
StringBuilder sbQuery = new StringBuilder();
sbQuery.append("select t1.member_id, t1.reservations_performance, t2.reservations_original\n");
sbQuery.append("from (\n");
sbQuery.append("	select \n");
sbQuery.append("		member_id, \n");
sbQuery.append("		sum(reservation_count) as reservations_performance\n");
sbQuery.append("	from {{marketing.schema}}.vw_s_member_performance\n");
sbQuery.append("	group by member_id\n");
sbQuery.append(") t1\n");
sbQuery.append("join (\n");
sbQuery.append("	select \n");
sbQuery.append("		member_id,\n");
sbQuery.append("		count(reservation_id) as reservations_original\n");
sbQuery.append("	from {{marketing.schema}}.s_reservation sr\n");
sbQuery.append("	where (sr.cancel_date is null)\n");
sbQuery.append("	group by member_id\n");
sbQuery.append(") t2\n");
sbQuery.append("on t1.member_id = t2.member_id\n");
sbQuery.append("where t1.reservations_performance <> t2.reservations_original\n");
sbQuery.append(";\n");
String query = sbQuery.toString().replaceAll("\\{\\{marketing.schema\\}\\}", context.Redshift_Marketing_Schema);

emailContent.append("<h3>Mismatched Reservation Counts</h3>\n");
emailContent.append("<p>There is a total of<b> " + sharedMap.get("members_with_mismatched_reservations") + " members</b> with mismatched reservation counts.</p>\n");
emailContent.append("<p>To view the detail information, please run the following SQL query: </p>\n");
emailContent.append("<pre><code>" + query + "</code></pre>\n");
...
if(emailContent.length() > 0) {
	emailContent.insert(0, "<head><style>table, th, td {border: 1px solid black;border-collapse:collapse;border-spacing: 0px; padding: 5px;} </style></head>\n");
	emailContent.insert(0, "<html><body>\n");
...
end_Hash.put("tJava_5", System.currentTimeMillis());
   			...
   			if (((StringBuilder) globalMap.get("EmailContent")).length() > 0) {
 ...
 */
    ...
    String subject_tSendMail_1 = (NotificationUtils.jobStatusSubject(context.environment, true, "Data Consistency Monitor - " + TalendDate.formatDate("yyyy-MM-dd", DateUtils.runDate(context.RunDate))));
	...
	String message_tSendMail_1 = ((globalMap.get("EmailContent").toString()) == null || "".equals(globalMap.get("EmailContent").toString())) ? "\"\"" : (globalMap.get("EmailContent").toString()) ;
 ...
 */
    ...
    public static void main(String[] args){
        final Data_Consistency_Monitor Data_Consistency_MonitorClass = new Data_Consistency_Monitor();

        int exitCode = Data_Consistency_MonitorClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Data_Consistency_Monitor.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/data_consistency_monitor_0_1/contexts/"+contextStr+".properties");
            ...
            }catch(ParseException e)
            {
                ...
                context.sendMailCc_RatePlanMapping=(String) context.getProperty("sendMailCc_RatePlanMapping");
				    context.setContextType("sendMailCc_SchemaMonitor", "id_String");
				
                context.sendMailCc_SchemaMonitor=(String) context.getProperty("sendMailCc_SchemaMonitor");
        ...
        if (parentContextMap != null && !parentContextMap.isEmpty()) {if (parentContextMap.containsKey("RunDate")) {
            ...
            }if (parentContextMap.containsKey("sendMailCc_SchemaMonitor")) {
                context.sendMailCc_SchemaMonitor = (String) parentContextMap.get("sendMailCc_SchemaMonitor");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Data_Consistency_Monitor");
        }
...
/************************************************************************************************
 *     190478 characters generated by Talend Open Studio for Data Integration 
