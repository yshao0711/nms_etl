...
package zipcar_marketingdw.processstaging_clicks_0_1;
...
/**
 * Job: ProcessStaging_Clicks Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status 
 */
public class ProcessStaging_Clicks implements TalendJob {
...
public String getRedshift_Staging_Server(){
	...
	private final String jobName = "ProcessStaging_Clicks";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_LrIvgFraEei_EJm82Z_azQ", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				ProcessStaging_Clicks.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(ProcessStaging_Clicks.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_ProcessStaging_Clicks = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Clicks = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Clicks.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Clicks.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Clicks = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Clicks = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Clicks, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Clicks, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_ProcessStaging_Clicks) {
 ...
 */
	...
	Object obj_tRunJob_1 = null;
		...
		obj_tRunJob_1 = "salesforce_clicks";
		if(obj_tRunJob_1!=null) {
		...
		obj_tRunJob_1 = "r_salesforce_click";
		if(obj_tRunJob_1!=null) {
...
"\nTRUNCATE {{staging.schema}}.salesforce_clicks;\n",
"staging.schema",  StringHandling.DQUOTE(context.Redshift_Staging_Schema)
);
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_1", "", Thread.currentThread().getId() + "", "INFO","","Historical Records Inserted for r_salesforce_click on: " + DateUtils.dateToSqlStr(DateUtils.runDate(context.RunDate)),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_1", 3, "Historical Records Inserted for r_salesforce_click on: " + DateUtils.dateToSqlStr(DateUtils.runDate(context.RunDate)), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_1_WARN_MESSAGES", "Historical Records Inserted for r_salesforce_click on: " + DateUtils.dateToSqlStr(DateUtils.runDate(context.RunDate))); 
 ...
 */
    ...
    public static void main(String[] args){
        final ProcessStaging_Clicks ProcessStaging_ClicksClass = new ProcessStaging_Clicks();

        int exitCode = ProcessStaging_ClicksClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = ProcessStaging_Clicks.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/processstaging_clicks_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : ProcessStaging_Clicks");
        }
