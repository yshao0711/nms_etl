...
package zipcar_marketingdw.process_member_segment_report_0_1;
...
/**
 * Job: Process_Member_Segment_Report Purpose: Computes member segment report<br>
 * Description: Computes member segment report <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Process_Member_Segment_Report implements TalendJob {
...
public Boolean getIsSubJob(){
	...
	private final String jobName = "Process_Member_Segment_Report";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_PkmhsAhXEeioVoIDIVCDcA", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Process_Member_Segment_Report.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Process_Member_Segment_Report.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
Logging.componentName(jobName, "tWarn_3", "Log_Reset_Rows");
...
public static class external_configStruct implements routines.system.IPersistableRow<external_configStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_Member_Segment_Report = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Process_Member_Segment_Report = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Process_Member_Segment_Report.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Process_Member_Segment_Report.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_Member_Segment_Report = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_Member_Segment_Report = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Process_Member_Segment_Report, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Process_Member_Segment_Report, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_Member_Segment_Report) {
...
"\nDELETE FROM {{marketing.schema}}.s_rpt_member_segment  \nWHERE end_date = {{endDate}}\n",
...
if(deleted_rows != null) { 
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_3", "", Thread.currentThread().getId() + "", "INFO","","Deleted Current Day Records for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_3", 3, "Deleted Current Day Records for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_3_WARN_MESSAGES", "Deleted Current Day Records for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE"))); 
...
"\nCREATE TEMP TABLE calc_date AS\n-- get the calc dates for any run date\n-- to_date('2018-02-01', 'YYYY-MM-DD')\nSELECT\n		trunc(date_trunc('week', to_date({{runDate}}, 'YYYY-MM-DD'))) - 1 AS sunday,\n		trunc(date_trunc('week', to_date({{runDate}}, 'YYYY-MM-DD'))) - 8 AS prev_sunday,\n		trunc(date_trunc('week', to_date({{runDate}}, 'YYYY-MM-DD'))) - 7 AS prev_monday\n;\nCREATE TEMP TABLE this_weeks_segments AS\n	SELECT\n		mem.member_id,\n		rp.segment_class,\n		rp.plan_category,\n		sma.primary_account_id,\n		sma.primary_affiliate_id,\n		mem.current_country,\n		lm.metro_name\n	FROM {{marketing.schema}}.d_member mem\n	JOIN calc_date cd ON 1=1\n	LEFT OUTER JOIN {{marketing.schema}}.s_member_activity sma ON mem.person_id = sma.person_id AND sma.end_date = cd.sunday\n	LEFT OUTER JOIN {{marketing.schema}}.vw_r_rate_plan rp ON sma.primary_rate_plan_id = rp.rate_plan_id\n	LEFT OUTER JOIN {{marketing.schema}}.l_metro lm ON sma.metro_id = lm.metro_id\n	WHERE mem.end_date = '3199-12-31'\n;\nCREATE TEMP TABLE last_weeks_segments AS\n	SELECT\n		mem.member_id,\n		rp.segment_class,\n		rp.plan_category,\n		sma.primary_account_id,\n		sma.primary_affiliate_id,\n		mem.current_country,\n		lm.metro_name\n	FROM {{marketing.schema}}.d_member mem\n	JOIN calc_date cd ON 1=1\n	LEFT OUTER JOIN {{marketing.schema}}.s_member_activity sma ON mem.person_id = sma.person_id AND sma.end_date = cd.prev_sunday\n	LEFT OUTER JOIN {{marketing.schema}}.vw_r_rate_plan rp ON sma.primary_rate_plan_id = rp.rate_plan_id\n	LEFT OUTER JOIN {{marketing.schema}}.l_metro lm ON sma.metro_id = lm.metro_id\n	WHERE mem.end_date = '3199-12-31'\n;\nCREATE TEMP TABLE member_segments AS\nSELECT member_id, segment_class, plan_category, aag.affiliate_grouping, aag.type, current_country, metro_name\nFROM (\n	SELECT\n		tws.member_id,\n		COALESCE(tws.segment_class, lts.segment_class) as segment_class,\n		COALESCE(tws.plan_category, lts.plan_category) as plan_category,\n		COALESCE(tws.primary_account_id, lts.primary_account_id) as primary_account_id,\n		COALESCE(tws.primary_affiliate_id, lts.primary_affiliate_id) as primary_affiliate_id,\n		COALESCE(tws.current_country, lts.current_country) as current_country,\n		COALESCE(tws.metro_name, lts.metro_name) as metro_name\n	FROM this_weeks_segments tws\n	INNER JOIN last_weeks_segments lts ON tws.member_id = lts.member_id\n) tt\nLEFT OUTER JOIN {{marketing.schema}}.l_affiliate_account_grouping aag ON (tt.primary_account_id = aag.account_id OR tt.primary_affiliate_id = aag.affiliate_id)\nWHERE segment_class IS NOT NULL AND plan_category IS NOT NULL\n;\nCREATE TEMP TABLE end_member AS\n    SELECT DISTINCT dm.member_id, dm.person_id\n    FROM {{marketing.schema}}.d_member AS dm\n    JOIN calc_date cd ON 1=1\n    WHERE dm.start_date <= cd.sunday\n    AND dm.end_date >= cd.sunday\n    AND dm.is_active = TRUE\n;\nCREATE TEMP TABLE begin_member AS \n    SELECT DISTINCT dm.member_id, dm.person_id\n    FROM {{marketing.schema}}.d_member AS dm\n    JOIN calc_date cd ON 1=1\n    WHERE dm.start_date <= cd.prev_monday\n    AND dm.end_date >= cd.prev_monday\n    AND dm.is_active = TRUE\n;\nCREATE TEMP TABLE new_member AS \n	SELECT * FROM end_member\n	MINUS\n	SELECT * FROM begin_member\n;\nCREATE TEMP TABLE begin_n_new_member AS \n	SELECT * FROM begin_member\n	UNION\n	SELECT * FROM new_member\n;\nCREATE TEMP TABLE left_member AS \n	SELECT * FROM begin_member\n	MINUS\n	SELECT * FROM end_member\n;\nCREATE TEMP TABLE driving_member AS \n	SELECT bnm.member_id\n	FROM begin_n_new_member bnm\n	JOIN calc_date cd ON 1=1\n	JOIN {{marketing.schema}}.s_reservation sr\n	ON bnm.member_id = sr.member_id\n	and sr.start_date <= cd.sunday\n	and sr.end_date >= cd.prev_monday\n	and sr.cancel_date IS NULL\n 	GROUP BY bnm.member_id\n;\nCREATE TEMP TABLE agg_member_begin AS \n	SELECT ms.segment_class, ms.plan_category, ms.affiliate_grouping, ms.type, \n		ms.current_country, ms.metro_name, count(*) AS member_count\n	FROM begin_member bm\n	JOIN member_segments ms\n	ON bm.member_id = ms.member_id\n	GROUP BY ms.segment_class, ms.plan_category, affiliate_grouping, type, current_country, metro_name\n;\nCREATE TEMP TABLE agg_member_new AS \n	SELECT ms.segment_class, ms.plan_category, ms.affiliate_grouping, ms.type, \n		ms.current_country, ms.metro_name, count(*) AS member_count\n	FROM new_member nm\n	JOIN member_segments ms\n	ON nm.member_id = ms.member_id\n	GROUP BY ms.segment_class, ms.plan_category, affiliate_grouping, type, current_country, metro_name\n;\nCREATE TEMP TABLE agg_member_left AS \n	SELECT ms.segment_class, ms.plan_category, ms.affiliate_grouping, ms.type, \n		ms.current_country, ms.metro_name, count(*) AS member_count\n	FROM left_member lm\n	JOIN member_segments ms\n	ON lm.member_id = ms.member_id\n	GROUP BY ms.segment_class, ms.plan_category, affiliate_grouping, type, current_country, metro_name\n;\nCREATE TEMP TABLE agg_member_end AS \n	SELECT ms.segment_class, ms.plan_category, ms.affiliate_grouping, ms.type, \n		ms.current_country, ms.metro_name, count(*) AS member_count\n	FROM end_member em\n	JOIN member_segments ms\n	ON em.member_id = ms.member_id\n	GROUP BY ms.segment_class, ms.plan_category, affiliate_grouping, type, current_country, metro_name\n;\nCREATE TEMP TABLE agg_member_driving AS \n	SELECT ms.segment_class, ms.plan_category, ms.affiliate_grouping, ms.type, \n		ms.current_country, ms.metro_name, count(*) AS member_count\n	FROM driving_member dm\n	JOIN member_segments ms\n	ON dm.member_id = ms.member_id\n	GROUP BY ms.segment_class, ms.plan_category, affiliate_grouping, type, current_country, metro_name\n;\nCREATE TEMP TABLE agg_member_summary AS \n	SELECT\n		segment_class,\n		plan_category,\n		affiliate_grouping,\n		type,\n		current_country,\n		metro_name,\n		SUM(revenue) AS revenue,\n		SUM(reservation_hours) AS reservation_hours,\n		SUM(reservations) AS reservations,\n		AVG(days_since_last_reservation) AS days_since_last_reservation\n	FROM (\n		SELECT\n			ms.member_id,\n			ms.segment_class,\n			ms.plan_category,\n			ms.affiliate_grouping,\n			ms.type,\n			ms.current_country,\n			ms.metro_name,\n			smp.sum_of_revenue as revenue,\n			smp.total_reservation_time as reservation_hours,\n			smp.reservation_count as reservations,\n			latest_res.days_since_last_reservation\n		FROM end_member em\n		JOIN calc_date cd ON 1=1\n		JOIN member_segments ms\n		ON em.member_id = ms.member_id\n		JOIN {{marketing.schema}}.s_member_performance smp\n		ON em.person_id = smp.person_id\n		AND smp.end_date = cd.sunday\n		LEFT JOIN (\n			SELECT\n				member_id,\n				reservation_id,\n				datediff(day, start_date, cd.sunday) AS days_since_last_reservation\n			FROM (\n				SELECT\n					res.member_id,\n					res.reservation_id,\n					res.start_date,\n					ROW_NUMBER() OVER (PARTITION BY res.member_id ORDER BY res.start_date DESC) AS row_num\n				FROM {{marketing.schema}}.s_reservation res\n				JOIN calc_date cd ON 1=1\n				WHERE	res.start_date <= cd.sunday\n				AND (res.cancel_date is null) -- Not cancelled\n			)\n			JOIN calc_date cd ON 1=1\n			WHERE row_num = 1\n		) latest_res ON em.member_id = latest_res.member_id\n	)\n	GROUP BY segment_class, plan_category, affiliate_grouping, type, current_country, metro_name\n;\nINSERT INTO {{marketing.schema}}.s_rpt_member_segment\nSELECT\n	ms.segment_class,\n	ms.plan_category,\n	ms.affiliate_grouping,\n	ms.type,\n	ms.current_country,\n	ms.metro_name,\n	cd.prev_monday as start_date,\n	cd.sunday as end_date,\n	ms.revenue,\n	ms.reservation_hours,\n	ms.reservations,\n	ms.days_since_last_reservation,\n	nvl(mb.member_count, 0) AS begin_members,\n	nvl(mn.member_count, 0) AS new_members,\n	nvl(ml.member_count, 0) AS left_members,\n	nvl(md.member_count, 0) AS driving_members,\n	CASE WHEN nvl(mb.member_count, 0) + nvl(mn.member_count, 0) = 0 THEN 0.00\n	ELSE ROUND(nvl(md.member_count, 0) * 100.00 / (nvl(mb.member_count, 0) + nvl(mn.member_count, 0)), 2)\n	END AS pct_driving,\n	nvl(me.member_count, 0) AS end_members\nFROM agg_member_summary ms\nJOIN calc_date cd ON 1=1\nLEFT JOIN agg_member_begin mb\nON (\n	ms.segment_class = mb.segment_class \n	AND ms.plan_category = mb.plan_category \n	AND ((ms.affiliate_grouping = mb.affiliate_grouping) OR (ms.affiliate_grouping IS NULL AND mb.affiliate_grouping IS NULL))\n	AND ((ms.type = mb.type) OR (ms.type IS NULL AND mb.type IS NULL))\n	AND ((ms.current_country = mb.current_country) OR (ms.current_country IS NULL AND mb.current_country IS NULL))\n	AND ((ms.metro_name = mb.metro_name) OR (ms.metro_name IS NULL AND mb.metro_name IS NULL))\n)\nLEFT JOIN agg_member_new mn\nON (\n	ms.segment_class = mn.segment_class \n	AND ms.plan_category = mn.plan_category \n	AND ((ms.affiliate_grouping = mn.affiliate_grouping) OR (ms.affiliate_grouping IS NULL AND mn.affiliate_grouping IS NULL))\n	AND ((ms.type = mn.type) OR (ms.type IS NULL AND mn.type IS NULL))\n	AND ((ms.current_country = mn.current_country) OR (ms.current_country IS NULL AND mn.current_country IS NULL))\n	AND ((ms.metro_name = mn.metro_name) OR (ms.metro_name IS NULL AND mn.metro_name IS NULL))\n)\nLEFT JOIN agg_member_left ml\nON (\n	ms.segment_class = ml.segment_class \n	AND ms.plan_category = ml.plan_category \n	AND ((ms.affiliate_grouping = ml.affiliate_grouping) OR (ms.affiliate_grouping IS NULL AND ml.affiliate_grouping IS NULL))\n	AND ((ms.type = ml.type) OR (ms.type IS NULL AND ml.type IS NULL))\n	AND ((ms.current_country = ml.current_country) OR (ms.current_country IS NULL AND ml.current_country IS NULL))\n	AND ((ms.metro_name = ml.metro_name) OR (ms.metro_name IS NULL AND ml.metro_name IS NULL))\n)\nLEFT JOIN agg_member_driving md\nON (\n	ms.segment_class = md.segment_class \n	AND ms.plan_category = md.plan_category \n	AND ((ms.affiliate_grouping = md.affiliate_grouping) OR (ms.affiliate_grouping IS NULL AND md.affiliate_grouping IS NULL))\n	AND ((ms.type = md.type) OR (ms.type IS NULL AND md.type IS NULL))\n	AND ((ms.current_country = md.current_country) OR (ms.current_country IS NULL AND md.current_country IS NULL))\n	AND ((ms.metro_name = md.metro_name) OR (ms.metro_name IS NULL AND md.metro_name IS NULL))\n)\nLEFT JOIN agg_member_end me\nON (\n	ms.segment_class = me.segment_class \n	AND ms.plan_category = me.plan_category \n	AND ((ms.affiliate_grouping = me.affiliate_grouping) OR (ms.affiliate_grouping IS NULL AND me.affiliate_grouping IS NULL))\n	AND ((ms.type = me.type) OR (ms.type IS NULL AND me.type IS NULL))\n	AND ((ms.current_country = me.current_country) OR (ms.current_country IS NULL AND me.current_country IS NULL))\n	AND ((ms.metro_name = me.metro_name) OR (ms.metro_name IS NULL AND me.metro_name IS NULL))\n)\nORDER BY ms.segment_class, ms.plan_category, ms.affiliate_grouping, ms.type, ms.current_country, ms.metro_name\n;\n",
"marketing.schema", StringHandling.DQUOTE(context.Redshift_Marketing_Schema),
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_6", "", Thread.currentThread().getId() + "", "INFO","","Member segment data inserted into (s_rpt_member_segment) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_6", 3, "Member segment data inserted into (s_rpt_member_segment) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_6_WARN_MESSAGES", "Member segment data inserted into (s_rpt_member_segment) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE"))); 
 ...
 */
    ...
    public static void main(String[] args){
        final Process_Member_Segment_Report Process_Member_Segment_ReportClass = new Process_Member_Segment_Report();

        int exitCode = Process_Member_Segment_ReportClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Process_Member_Segment_Report.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/process_member_segment_report_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Process_Member_Segment_Report");
        }
...
/************************************************************************************************
 *     153520 characters generated by Talend Open Studio for Data Integration 
