...
package zipcar_marketingdw.process_monthly_location_segments_0_1;
...
/**
 * Job: Process_Monthly_Location_Segments Purpose: Computes member monthly location segmentation<br>
 * Description: Computes member monthly location segmentation from reservation data <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Process_Monthly_Location_Segments implements TalendJob {
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Process_Monthly_Location_Segments";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_WekD4GWxEei4tNjVPIAMGw", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Process_Monthly_Location_Segments.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Process_Monthly_Location_Segments.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class deleted_rowsStruct implements routines.system.IPersistableRow<deleted_rowsStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_Monthly_Location_Segments = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Location_Segments = new byte[0];
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Process_Monthly_Location_Segments) {
...
"\nDELETE FROM {{marketing.schema}}.s_member_location_segment_monthly\nWHERE end_date = {{endDate}}\n",
...
if(deleted_rows != null) { 
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_3", "", Thread.currentThread().getId() + "", "INFO","","Deleted Monthly Location Segment Data for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_3", 3, "Deleted Monthly Location Segment Data for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_3_WARN_MESSAGES", "Deleted Monthly Location Segment Data for Restart date: " + DateUtils.dateToSqlStr((Date) globalMap.get("END_DATE"))); 
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Location_Segments.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Location_Segments.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Location_Segments = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Location_Segments = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Location_Segments, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Process_Monthly_Location_Segments, 0, length, utf8Charset);
		}
		return strReturn;
	}
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_6", "", Thread.currentThread().getId() + "", "INFO","","Monthly member location segment data inserted into (s_member_location_segment_monthly) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_6", 3, "Monthly member location segment data inserted into (s_member_location_segment_monthly) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE")), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_6_WARN_MESSAGES", "Monthly member location segment data inserted into (s_member_location_segment_monthly) for Run Date: " + DateUtils.dateToSqlStr((Date) globalMap.get("RUN_DATE"))); 
 ...
 */
    ...
    public static void main(String[] args){
        final Process_Monthly_Location_Segments Process_Monthly_Location_SegmentsClass = new Process_Monthly_Location_Segments();

        int exitCode = Process_Monthly_Location_SegmentsClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Process_Monthly_Location_Segments.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/process_monthly_location_segments_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Process_Monthly_Location_Segments");
        }
...
/************************************************************************************************
 *     122955 characters generated by Talend Open Studio for Data Integration 
