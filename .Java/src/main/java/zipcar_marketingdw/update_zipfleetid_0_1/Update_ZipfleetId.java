...
package zipcar_marketingdw.update_zipfleetid_0_1;
...
/**
 * Job: Update_ZipfleetId Purpose: Updates the members zipfleet id<br>
 * Description: Uses the member_zipfleet_id assigned in the applications_final table. <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Update_ZipfleetId implements TalendJob {
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Update_ZipfleetId";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_gKSyoBoHEeemPJRlMR-KSA", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Update_ZipfleetId.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Update_ZipfleetId.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Update_ZipfleetId = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Update_ZipfleetId = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Update_ZipfleetId.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Update_ZipfleetId.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Update_ZipfleetId = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Update_ZipfleetId = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Update_ZipfleetId, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Update_ZipfleetId, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Update_ZipfleetId) {
...
"\nCREATE TEMP TABLE calc_member_zipfleet AS \nWITH member_app AS (\n	SELECT \n	    ma.*, \n	    decode(ma.application_approval_state, 'finished', 1, 2) as app_state_order \n     FROM {{marketing.schema}}.r_applications_final ma\n	JOIN {{marketing.schema}}.d_member cm ON\n		cm.member_id = ma.member_id\n	WHERE\n		ma.deleted_date IS NULL\n		AND cm.end_date = '3199-12-31'\n        AND ma.member_zipfleet_id IS NOT NULL\n),\nordered_app as (\n	SELECT \n		t1.member_id,\n		t1.member_zipfleet_id,\n		row_number() OVER (PARTITION BY member_id ORDER BY  app_state_order ASC, creation_date DESC, application_id DESC, finished_date DESC) row_num\n	FROM member_app t1\n),\nlatest_zipfleet_id as (\n	SELECT member_id, member_zipfleet_id from ordered_app\n	WHERE\n		row_num=1\n)\nSELECT lz.*, zf.country_iso\nFROM latest_zipfleet_id lz\nJOIN {{marketing.schema}}.vw_r_zipfleet zf\nON lz.member_zipfleet_id = zf.zipfleet_id\n",
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_2", "", Thread.currentThread().getId() + "", "INFO","","Calculated Current Member Zipfleet Id","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_2", 3, "Calculated Current Member Zipfleet Id", 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_2_WARN_MESSAGES", "Calculated Current Member Zipfleet Id"); 
...
"\nCREATE TEMP TABLE calc_updated_members AS \nWITH current_members AS (\n	SELECT * FROM {{marketing.schema}}.d_member\n	WHERE\n		end_date = '3199-12-31'\n)\nSELECT dm.* FROM current_members dm JOIN\ncalc_member_zipfleet mz ON\n	dm.member_id = mz.member_id\n	AND (dm.join_zipfleet_id IS NULL OR dm.join_zipfleet_id <> mz.member_zipfleet_id)\n",
"marketing.schema", context.Redshift_Marketing_Schema
);
...
"\nUPDATE calc_updated_members  SET\n    join_zipfleet_id = cmz.member_zipfleet_id,\n	join_country = cmz.country_iso,\n    start_date = {{current.date}},\n    imported_date = {{current.date}}\nFROM calc_member_zipfleet cmz\nWHERE\n     calc_updated_members.member_id=cmz.member_id\n",
"marketing.schema", context.Redshift_Marketing_Schema,
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_1", "", Thread.currentThread().getId() + "", "INFO","","Members updated zipfleet_id","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_1", 3, "Members updated zipfleet_id", 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_1_WARN_MESSAGES", "Members updated zipfleet_id"); 
 ...
 */
    ...
    public static void main(String[] args){
        final Update_ZipfleetId Update_ZipfleetIdClass = new Update_ZipfleetId();

        int exitCode = Update_ZipfleetIdClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Update_ZipfleetId.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/update_zipfleetid_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Update_ZipfleetId");
        }
...
/************************************************************************************************
 *     133904 characters generated by Talend Open Studio for Data Integration 
