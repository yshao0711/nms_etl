...
package zipcar_marketingdw.update_memberisemployee_0_1;
...
/**
 * Job: Update_MemberIsEmployee Purpose: Updates the member is_employee flag<br>
 * Description:  <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Update_MemberIsEmployee implements TalendJob {
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Update_MemberIsEmployee";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_qjEbMInPEee5n-XeCGSojw", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Update_MemberIsEmployee.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Update_MemberIsEmployee.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
Logging.componentName(jobName, "tWarn_2", "Log_CalcZipfleetId");
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Update_MemberIsEmployee = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberIsEmployee = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberIsEmployee.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberIsEmployee.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberIsEmployee = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberIsEmployee = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberIsEmployee, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberIsEmployee, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Update_MemberIsEmployee) {
...
"\nCREATE TEMP TABLE calc_member_employee AS \nWITH current_members AS (\n    SELECT\n        rm.member_id,\n        rm.last_name,\n        rm.email\n    FROM {{marketing.schema}}.vw_r_member rm\n    WHERE \n        rm.member_id in (\n            SELECT DISTINCT member_id FROM {{marketing.schema}}.d_member WHERE end_date='3199-12-31'\n        )\n),\nmember_with_employee_account as (\n    select distinct cm.member_id        \n    from current_members cm\n    join {{marketing.schema}}.vw_r_account_member_map amm\n    on cm.member_id = amm.member_id\n    join {{marketing.schema}}.vw_r_account_rate_plan_map arpm\n    on amm.account_id = arpm.account_id\n    join {{marketing.schema}}.vw_r_rate_plan rp\n    on arpm.rate_plan_id = rp.rate_plan_id\n    where lower(rp.plan_category)='employee'\n    and amm.start_date <= {{current.date}}\n    and (amm.end_date is null or amm.end_date >= {{current.date}})\n    and (amm.deleted_date is null or amm.deleted_date > {{current.date}})\n    and arpm.start_date <= {{current.date}}\n    and (arpm.end_date is null or arpm.end_date >= {{current.date}})\n    and (arpm.deleted_date is null or arpm.deleted_date > {{current.date}})\n    and (rp.deleted_date is null or rp.deleted_date > {{current.date}})\n)\nselect \n    cm.member_id,\n    CASE\n        WHEN mea.member_id is not null THEN TRUE\n        WHEN lower(cm.last_name) like '%zipcar%' THEN TRUE\n        WHEN lower(cm.email) like '%@zipcar.%' THEN TRUE\n        WHEN lower(cm.email) like '%@driver.car%' THEN TRUE\n        ELSE FALSE\n    END AS is_employee\nfrom current_members cm\nleft join member_with_employee_account mea\non cm.member_id = mea.member_id;\n",
...
"\nUPDATE calc_updated_members  SET\n    is_employee = cme.is_employee,\n    start_date = {{current.date}},\n    imported_date = {{current.date}}\nFROM calc_member_employee cme\nWHERE\n     calc_updated_members.member_id=cme.member_id\n",
"marketing.schema", context.Redshift_Marketing_Schema,
"current.date", DateUtils.dateToSqlStr((Date) globalMap.get("CURRENT_DATE"))
);
 ...
 */
    ...
    public static void main(String[] args){
        final Update_MemberIsEmployee Update_MemberIsEmployeeClass = new Update_MemberIsEmployee();

        int exitCode = Update_MemberIsEmployeeClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Update_MemberIsEmployee.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/update_memberisemployee_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Update_MemberIsEmployee");
        }
...
/************************************************************************************************
 *     134729 characters generated by Talend Open Studio for Data Integration 
