...
package zipcar_marketingdw.processstaging_sent_0_1;
...
/**
 * Job: ProcessStaging_Sent Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status 
 */
public class ProcessStaging_Sent implements TalendJob {
...
public String getRedshift_Staging_Server(){
	...
	private final String jobName = "ProcessStaging_Sent";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("__uIgEFrbEei_EJm82Z_azQ", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				ProcessStaging_Sent.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(ProcessStaging_Sent.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_ProcessStaging_Sent = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Sent = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Sent.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Sent.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Sent = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Sent = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Sent, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_ProcessStaging_Sent, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_ProcessStaging_Sent) {
 ...
 */
	...
	Object obj_tRunJob_1 = null;
		...
		obj_tRunJob_1 = "r_salesforce_sent";
		if(obj_tRunJob_1!=null) {
...
"\nTRUNCATE {{staging.schema}}.salesforce_sent;\n",
"staging.schema",  StringHandling.DQUOTE(context.Redshift_Staging_Schema)
);
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_1", "", Thread.currentThread().getId() + "", "INFO","","Historical Records Inserted for r_salesforce_sent on: " + DateUtils.dateToSqlStr(DateUtils.runDate(context.RunDate)),"", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_1", 3, "Historical Records Inserted for r_salesforce_sent on: " + DateUtils.dateToSqlStr(DateUtils.runDate(context.RunDate)), 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_1_WARN_MESSAGES", "Historical Records Inserted for r_salesforce_sent on: " + DateUtils.dateToSqlStr(DateUtils.runDate(context.RunDate))); 
 ...
 */
    ...
    public static void main(String[] args){
        final ProcessStaging_Sent ProcessStaging_SentClass = new ProcessStaging_Sent();

        int exitCode = ProcessStaging_SentClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = ProcessStaging_Sent.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/processstaging_sent_0_1/contexts/"+contextStr+".properties");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : ProcessStaging_Sent");
        }
