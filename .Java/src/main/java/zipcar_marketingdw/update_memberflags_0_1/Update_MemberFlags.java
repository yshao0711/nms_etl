...
package zipcar_marketingdw.update_memberflags_0_1;
...
/**
 * Job: Update_MemberFlags Purpose: Updates the Member Flags<br>
 * Description: Updates is_active, is_admin and is_multi_account, is_driver, is_member, has_service_account, has_nonservice_account <br>
 * @author user@talend.com
 * @version 6.4.1.20170623_1246
 * @status DEV
 */
public class Update_MemberFlags implements TalendJob {
...
public String getRedshift_Monitoring_Server(){
	...
	private final String jobName = "Update_MemberFlags";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils tStatCatcher_1 = new StatCatcherUtils("_qcAKINvZEeaFP4Us6dSDsA", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Update_MemberFlags.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Update_MemberFlags.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
Logging.componentName(jobName, "tWarn_2", "Summary");
Logging.componentName(jobName, "tWarn_1", "Log_CalcZipfleetId");
Logging.componentName(jobName, "tWarn_2", "Log_CalcUpdatedMembers");
...
public static class log_messageStruct implements routines.system.IPersistableRow<log_messageStruct> {
    final static byte[] commonByteArrayLock_ZIPCAR_MARKETINGDW_Update_MemberFlags = new byte[0];
    static byte[] commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberFlags = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberFlags.length) {
				if(length < 1024 && commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberFlags.length == 0) {
   					commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberFlags = new byte[1024];
				} else {
   					commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberFlags = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberFlags, 0, length);
			strReturn = new String(commonByteArray_ZIPCAR_MARKETINGDW_Update_MemberFlags, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ZIPCAR_MARKETINGDW_Update_MemberFlags) {
...
"\nCREATE TEMP TABLE calc_member_status AS \nWITH member_status as (\n    SELECT\n        member_id, \n        current_status\n    FROM {{marketing.schema}}.vw_r_member\n    WHERE \n        member_id IN (SELECT DISTINCT member_id FROM {{marketing.schema}}.d_member)\n),\nactive_accounts AS (\n    SELECT DISTINCT \n        famm.member_id,\n        famm.account_id    \n    FROM {{marketing.schema}}.vw_r_account_member_map famm\n    INNER JOIN {{marketing.schema}}.vw_r_account_rate_plan_map farpm ON \n        famm.account_id = farpm.account_id\n    INNER JOIN {{marketing.schema}}.vw_r_rate_plan frp ON \n        farpm.rate_plan_id = frp.rate_plan_id\n    INNER JOIN member_status dm ON \n        dm.member_id=famm.member_id\n    WHERE \n        lower(famm.status) = 'approved'\n        AND famm.start_date <= {{current.date}}\n        AND (famm.end_date is null or famm.end_date >=  {{current.date}} )\n        AND (famm.deleted_date is null or famm.deleted_date >  {{current.date}} )\n        AND farpm.start_date <= {{current.date}}\n        AND (farpm.end_date is null or farpm.end_date >=  {{current.date}} )\n        AND (farpm.deleted_date is null or farpm.deleted_date >  {{current.date}} )\n        AND (frp.deleted_date is null or frp.deleted_date > {{current.date}} )    \n),\nactive_account_count AS (\n    SELECT\n        ms.member_id,\n        COALESCE(ac.active_count, 0) as account_count\n    FROM member_status ms\n    LEFT JOIN \n    (\n        SELECT\n            member_id,\n            count(account_id) AS active_count\n        FROM active_accounts\n        GROUP BY member_id\n    ) ac ON\n        ms.member_id = ac.member_id\n),\nadmin_account_count AS (\n    SELECT\n        ms.member_id,\n        COALESCE(ad.admin_count, 0) as admin_count\n    FROM member_status ms\n    LEFT JOIN \n    (\n		SELECT\n		    aa.member_id,\n		    count(aa.account_id) AS admin_count\n		FROM {{marketing.schema}}.vw_r_account_admin aa\n		INNER JOIN {{marketing.schema}}.vw_r_account_member_map amm\n		   ON aa.account_id = amm.account_id               \n		INNER JOIN {{marketing.schema}}.vw_r_account_rate_plan_map arpm\n		   ON aa.account_id = arpm.account_id\n		INNER JOIN {{marketing.schema}}.vw_r_rate_plan rp\n		   ON arpm.rate_plan_id = rp.rate_plan_id\n		WHERE aa.member_id IN (SELECT member_id FROM {{marketing.schema}}.d_member GROUP BY member_id)\n		AND {{current.date}} BETWEEN aa.start_date AND NVL(aa.end_date, {{current.date}} + 3)+1\n		AND (aa.deleted_date IS NULL OR aa.deleted_date > {{current.date}})\n		AND {{current.date}} BETWEEN amm.start_date AND NVL(amm.end_date, {{current.date}} + 3)+1\n		AND (amm.deleted_date IS NULL OR amm.deleted_date > {{current.date}})\n		AND {{current.date}} BETWEEN arpm.start_date AND NVL(arpm.end_date, {{current.date}} + 3)+1\n		AND (arpm.deleted_date IS NULL OR arpm.deleted_date > {{current.date}})\n		AND (rp.deleted_date IS NULL OR rp.deleted_date > {{current.date}})      \n		GROUP BY aa.member_id\n    ) ad ON\n        ms.member_id = ad.member_id\n),\nservice_account_member as (\n    select amm.member_id, ac.service_account_p\n    from {{marketing.schema}}.vw_r_account_member_map amm\n    join {{marketing.schema}}.vw_r_account ac\n    on amm.account_id = ac.account_id\n    where amm.member_id IN (SELECT DISTINCT member_id FROM {{marketing.schema}}.d_member)\n    and amm.status = 'approved'\n    and amm.start_date <= {{current.date}}\n    and (amm.end_date is null or amm.end_date >= {{current.date}})\n    and (amm.deleted_date is null or amm.deleted_date > {{current.date}})\n),\nmember_has_service_account as (\n    select distinct member_id\n    from service_account_member\n    where service_account_p = 't'\n),\nmember_has_nonservice_account as (\n    select distinct member_id\n    from service_account_member\n    where service_account_p = 'f'\n)\nSELECT\n    ms.member_id,\n    ms.current_status,\n    ac.account_count,\n    CASE\n        WHEN ms.current_status <> 'suspended' AND ac.account_count > 0 THEN TRUE\n        ELSE FALSE\n    END AS is_active,\n    CASE\n        WHEN ac.account_count > 0 THEN TRUE\n        ELSE FALSE\n    END AS is_member,\n    CASE\n        WHEN ac.account_count > 1 THEN TRUE\n        ELSE FALSE\n    END AS is_multi_account,    \n    ad.admin_count,\n    CASE\n        WHEN ad.admin_count > 0 THEN TRUE\n        ELSE FALSE\n    END AS is_admin,\n    CASE\n        WHEN msa.member_id is not null THEN TRUE\n        ELSE FALSE\n    END AS has_service_account,\n    CASE\n        WHEN mnsa.member_id is not null THEN TRUE\n        ELSE FALSE\n    END AS has_nonservice_account\nFROM member_status ms\nJOIN active_account_count ac ON\n     ms.member_id = ac.member_id\nJOIN admin_account_count ad ON\n    ms.member_id = ad.member_id\nLEFT JOIN member_has_service_account msa ON\n    ms.member_id = msa.member_id\nLEFT JOIN member_has_nonservice_account mnsa ON\n    ms.member_id = mnsa.member_id\n",
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_1", "", Thread.currentThread().getId() + "", "INFO","","Calculated Current Member Zipfleet Id","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_1", 3, "Calculated Current Member Zipfleet Id", 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_1_WARN_MESSAGES", "Calculated Current Member Zipfleet Id"); 
...
"\nCREATE TEMP TABLE calc_updated_members AS \nWITH current_members AS (\n	SELECT * FROM {{marketing.schema}}.d_member\n	WHERE\n		end_date = '3199-12-31'\n)\nSELECT dm.* FROM current_members dm JOIN\ncalc_member_status mz ON\n	dm.member_id = mz.member_id\n	AND \n    (\n        (dm.is_active IS NULL OR dm.is_active <> mz.is_active)\n		OR(dm.is_member IS NULL OR dm.is_member <> mz.is_member)\n        OR (dm.is_multi_account IS NULL OR dm.is_multi_account <> mz.is_multi_account)\n        OR (dm.is_admin IS NULL OR dm.is_admin <> mz.is_admin)\n		OR (dm.has_service_account IS NULL OR dm.has_service_account <> mz.has_service_account)\n		OR (dm.has_nonservice_account IS NULL OR dm.has_nonservice_account <> mz.has_nonservice_account)\n    )\n",
"marketing.schema", context.Redshift_Marketing_Schema
);
...
"\nUPDATE calc_updated_members  SET\n    is_active = cms.is_active,\n	is_member = cms.is_member,\n    is_multi_account = cms.is_multi_account,\n    is_admin = cms.is_admin,\n	has_service_account = cms.has_service_account,\n	has_nonservice_account = cms.has_nonservice_account,\n    start_date = {{current.date}},\n    imported_date = {{current.date}}\nFROM calc_member_status cms\nWHERE\n     calc_updated_members.member_id=cms.member_id\n",
"marketing.schema", context.Redshift_Marketing_Schema,
"current.date", DateUtils.dateToSqlStr((Date) globalMap.get("CURRENT_DATE"))
);
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_2", "", Thread.currentThread().getId() + "", "INFO","","Calculated Updated Member Records","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_2", 3, "Calculated Updated Member Records", 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_2_WARN_MESSAGES", "Calculated Updated Member Records"); 
...
"\nUPDATE {{marketing.schema}}.d_member SET\n    is_active = calc.is_active,\n	is_member = calc.is_member,\n    is_multi_account =  calc.is_multi_account ,\n    is_admin = calc.is_admin,\n	has_service_account = calc.has_service_account,\n	has_nonservice_account = calc.has_nonservice_account\nFROM calc_updated_members calc\nWHERE\n     d_member.member_id=calc.member_id\n     AND \n     (\n         (d_member.is_active IS NULL OR d_member.is_active <> calc.is_active)\n		 OR (d_member.is_member IS NULL OR d_member.is_member <> calc.is_member)\n         OR (d_member.is_multi_account IS NULL OR d_member.is_multi_account <> calc.is_multi_account)\n         OR (d_member.is_admin IS NULL OR d_member.is_admin <> calc.is_admin)\n		 OR (d_member.has_service_account IS NULL OR d_member.has_service_account <> calc.has_service_account)\n		 OR (d_member.has_nonservice_account IS NULL OR d_member.has_nonservice_account <> calc.has_nonservice_account)\n     )\n     AND d_member.imported_date = {{current.date}}\n     AND d_member.end_date='3199-12-31'\n",
"marketing.schema", context.Redshift_Marketing_Schema,
"current.date", DateUtils.dateToSqlStr((Date) globalMap.get("CURRENT_DATE"))
);
...
"\nINSERT INTO {{marketing.schema}}.d_member\n    SELECT DISTINCT calc.* FROM calc_updated_members calc\n    JOIN {{marketing.schema}}.d_member dm ON\n        dm.end_date = '3199-12-31'\n        AND dm.member_id = calc.member_id\n        AND \n        (\n            (dm.is_active IS NULL OR dm.is_active <> calc.is_active)\n			OR (dm.is_member IS NULL OR dm.is_member <> calc.is_member)\n            OR (dm.is_multi_account IS NULL OR dm.is_multi_account <> calc.is_multi_account)\n            OR (dm.is_admin IS NULL OR dm.is_admin <> calc.is_admin)\n			OR (dm.has_service_account IS NULL OR dm.has_service_account <> calc.has_service_account)\n			OR (dm.has_nonservice_account IS NULL OR dm.has_nonservice_account <> calc.has_nonservice_account)\n        )\n	    AND dm.imported_date < {{current.date}}\n;\nUPDATE {{marketing.schema}}.d_member SET\n    end_date={{inactive.date}}\nFROM calc_updated_members calc\nWHERE\n    d_member.end_date = '3199-12-31'\n    AND d_member.member_id = calc.member_id\n    AND \n    (\n        (d_member.is_active IS NULL OR d_member.is_active <> calc.is_active)\n		OR (d_member.is_member IS NULL OR d_member.is_member <> calc.is_member)\n        OR (d_member.is_multi_account IS NULL OR d_member.is_multi_account <> calc.is_multi_account)\n        OR (d_member.is_admin IS NULL OR d_member.is_admin <> calc.is_admin)\n		OR (d_member.has_service_account IS NULL OR d_member.has_service_account <> calc.has_service_account)\n		OR (d_member.has_nonservice_account IS NULL OR d_member.has_nonservice_account <> calc.has_nonservice_account)\n    )\n	AND d_member.imported_date < {{current.date}}\n\n",
"marketing.schema", context.Redshift_Marketing_Schema,
"current.date", DateUtils.dateToSqlStr((Date) globalMap.get("CURRENT_DATE")),
 ...
 */
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_3", "", Thread.currentThread().getId() + "", "INFO","","Members updated account info","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_3", 3, "Members updated account info", 42);
	tLogCatcher_1Process(globalMap);
globalMap.put("tWarn_3_WARN_MESSAGES", "Members updated account info"); 
 ...
 */
    ...
    public static void main(String[] args){
        final Update_MemberFlags Update_MemberFlagsClass = new Update_MemberFlags();

        int exitCode = Update_MemberFlagsClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Update_MemberFlags.class.getClassLoader().getResourceAsStream("zipcar_marketingdw/update_memberflags_0_1/contexts/"+contextStr+".properties");
...
try {
errorCode = null;tRedshiftRow_1Process(globalMap);
if(!"failure".equals(status)) { status = "end"; }
}catch (TalendException e_tRedshiftRow_1) {
globalMap.put("tRedshiftRow_1_SUBPROCESS_STATE", -1);

e_tRedshiftRow_1.printStackTrace();
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Update_MemberFlags");
        }
...
/************************************************************************************************
 *     140185 characters generated by Talend Open Studio for Data Integration 
